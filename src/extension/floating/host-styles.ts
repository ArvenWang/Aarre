...
.bar { position:fixed; display:flex; flex-direction:column; justify-content:center; padding:3px; gap:2px; pointer-events:auto; }
.bar button { display:grid; place-items:center; flex:1; width:100%; min-height:0; padding:0; border:0; border-radius:9px; background:transparent; color:var(--dock-ink); cursor:pointer; outline:none; transition:background 120ms,color 120ms; }
...
.bar button[aria-pressed="true"] { color:var(--dock-accent); }
.bar button[aria-pressed="true"] svg { fill:color-mix(in srgb,var(--dock-accent) 14%,transparent); }
...
:host([data-theme="dark"]) { --dock-bg:#16181b; --dock-ink:#eef0f2; --dock-line:#34383c; --dock-hover:#24282c; --dock-accent:#48c6b6; }
