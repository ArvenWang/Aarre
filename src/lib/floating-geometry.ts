...
import type { DockSide } from "../shared/suite-dock/contract";
...
export interface FloatingPosition { width: number; handleRatio?: number; side?: DockSide }
...
export function floatingRects(position: FloatingPosition, viewport: Viewport, handleHeight = BAR_HEIGHT, dragRatio?: number) {
  return dockRects(viewport, position.width, handleHeight, position.handleRatio, position.side, dragRatio);
}
...
export function floatingSaveRect(viewport: Viewport, contentHeight = SAVE_PANEL_INITIAL_HEIGHT, position = defaultFloatingPosition, handleHeight = BAR_HEIGHT) {
  ...
  return { x: position.side === "left" ? bar.x : bar.x + bar.width - width, y: Math.max(menu.y, Math.min(menu.y + menu.height - height, bar.y + (bar.height - height) / 2)), width, height };
}
