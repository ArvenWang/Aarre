...
export function FloatingSettingsSection() {
  ...
  useEffect(() => { void getFloatingSettings().then(setSettings).catch(() => setError("无法读取悬浮球设置，请重试。")); }, []);
  ...
  return <section className="settings-section" aria-labelledby="floating-settings-title">
    ...
    <div className="settings-toggle-row"><div><strong>显示悬浮球</strong><small>普通网页上的快捷入口，可拖动到左右边缘。</small></div>
      <Switch checked={settings?.enabled ?? false} disabled={busy || !settings} label="显示悬浮球" onChange={(enabled) => settings && void save({ ...settings, enabled }, !enabled)} />
    </div>
    {host && settings && <div className="settings-toggle-row"><div><strong>在此网站显示</strong><small>{host}</small></div>
      <Switch checked={!settings.hiddenHosts.includes(host)} disabled={busy} label={`在 ${host} 显示悬浮球`}
    ...
    {settings && <div className="floating-settings-actions">
      <Button variant="tertiary" size="sm" disabled={busy} onClick={() => void save({ ...settings, position: defaultFloatingPosition })}>重置位置和大小</Button>
