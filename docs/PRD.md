...
| M2 立差异 | 做出竞品结构性做不到的东西 | F7–F12 | 20 个真实用户里有人主动说「离不开了」 |
...
- **历史决策（已由 2026-08-04 用户决定取代）：** 原方案排除第三方图标服务。当前仅在站点自身候选全部失败后对非敏感域名启用 Google S2 / DuckDuckGo Icons，默认开启、设置可关闭，并在隐私页如实披露；完整 URL、正文、Cookie、敏感及用户排除域名不外发。
...
整理提案不再使用 `topics`、标签或摘要生成主题归类和批量移动建议；保存单条书签时的文件夹建议仍保留为独立能力。
...
把整理成本摊到每次收藏，而不是攒到年底再来一次大扫除。该建议只服务于当前保存动作，不再进入批量整理提案。
...
- **第三方图标服务仅作受控兜底（2026-08-04 用户决定）。** Google S2 / DuckDuckGo Icons 只接收非敏感 hostname，且仅在站点自身候选全部失败后调用；用户可在设置关闭。仍不采用 unavatar 或其他域名画像服务。
