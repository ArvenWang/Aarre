...
- 管理页顶部只保留一行 Aarre 标识；收藏搜索位于收藏库自身工具栏，不承担全局网页搜索。输入草稿与已提交查询分离，只有提交后才改变列表、高亮、相关度排序和 URL。
...
- 快照仅截当前前台、非无痕、已收藏且加载稳定的页面；普通浏览只补缺图或静默刷新已满 7 天的截图。无痕、内网、银行、支付、医疗和用户排除页面同时禁止正文读取、AI 调用和截图。
