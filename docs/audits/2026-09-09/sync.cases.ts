...
describe("September audit: required sync recovery behavior", () => {
  it("A01 does not advance the cursor past changes that were never committed", async () => {
    ...
    console.log("A01", { cursor: values[CURSOR], localDataPersisted: Boolean(local) });
    expect(values[CURSOR] === 10 || Boolean(local)).toBe(true);
  });
  ...
  it("A06 does not upload unchanged old image bytes over a newer remote cover", async () => {
    ...
    expect((await getLocalResource(key))?.thumbnailDataUrl).toBe("data:image/webp;base64,AQID");
  ...
  it("A10 sends an explicit clear when the user deletes their note and last tag", () => {
    const payload = resourceCloudPayload(resource("clear-fields", { userNote: "", tags: [], tagsSource: "user" }));
