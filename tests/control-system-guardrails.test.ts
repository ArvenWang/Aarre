...
const fluidControlsUrl = new URL(
  "../src/ui/components/FluidControls.tsx",
  import.meta.url
);
...
describe("destructive actions are visibly destructive", () => {
  it("paints the confirm button with the negative token, not ink", async () => {
    ...
    const danger = rule(css, ".button-danger");

    expect(danger).toContain("background: var(--negative)");
    expect(danger).not.toContain("var(--ink)");
  });

  it("offers a quiet destructive entry point that is still red", async () => {
    const css = await readFile(baseCssUrl, "utf8");
    const quiet = rule(css, ".button-danger-quiet");

    expect(quiet).toContain("color: var(--negative)");
...
describe("cloud settings stay local-first", () => {
  it("returns the explicit scope immediately and syncs it before restoring older cloud state", async () => {
    const source = await readFile(backgroundUrl, "utf8");
    const helper = source.slice(
      source.indexOf("async function syncAfterExplicitCloudSettings"),
      source.indexOf("async function getResources")
    );
    const handler = source.slice(
      source.indexOf('case "SAVE_CLOUD_SETTINGS"'),
      source.indexOf('case "GET_CLOUD_USAGE"')
    );

    expect(helper.indexOf("syncCloudScopeSetting(settings)")).toBeLessThan(
      helper.indexOf("restoreDurableCloudState({ skipCloudScope: true })")
    );
    expect(handler).toContain("void syncAfterExplicitCloudSettings(next)");
    expect(handler).not.toContain("await syncAfterExplicitCloudSettings(next)");
    expect(handler.indexOf('method: "DELETE"')).toBeLessThan(
...
describe("only one system paints a given control", () => {
  it("offers a variant that paints nothing at all", async () => {
    ...
    expect(variants).toContain("unstyled:");
    // An empty class list is the whole point: a painted variant writes its
    // hover state as `hover:bg-*`, and a pseudo-class selector outranks the
    // plain class the project CSS uses on the very same element.
    expect(variants).toMatch(/unstyled:\s*""/);
  ...
  it("lets project CSS own the resurfacing row's padding and hover fill", async () => {
    ...
    const row = source.slice(
      source.indexOf("{contextResurfacing.map"),
      source.indexOf("</section>", source.indexOf("{contextResurfacing.map"))
    );

    expect(row).toContain('variant="unstyled"');
  ...
  it("keeps every CSS-driven button in src/ui on that variant", async () => {
    const classNames = await projectClassNames();
    ...
    for (const file of await collectTsx(uiDirectoryUrl.pathname)) {
      ...
      for (const match of source.matchAll(/<Button\b([\s\S]*?)>/g)) {
        ...
        if (cssDriven.length && !/variant="unstyled"/.test(attributes)) {
          offenders.push(`${file.split("/src/")[1]}: ${cssDriven.join(" ")}`);
        }
      }
    }
  ...
  it("does not mix the ghost hover paint into CSS-owned tag submit buttons", async () => {
    ...
    expect(fields).toContain('variant="unstyled"');
...
describe("nested surfaces use a tokenized radius ladder", () => {
  it("keeps shell, module, inset and control tiers explicit", async () => {
    ...
    expect(base).toContain("height: var(--control-h-button)");
    expect(base).toContain("min-height: var(--control-h-button)");
    expect(shape).toContain('button: "rounded-[var(--radius-control)]"');
    expect(sidepanel).toContain(
      ".organization-notice-banner .button {\n  border-radius: var(--radius-control);",
    );
    expect(sidepanel).toContain(
      ".settings-change-list article .button {\n  border-radius: var(--radius-control);",
    );
