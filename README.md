...
| 页面截图 | Aarre | 当前 Chrome 配置文件本地，永不上传 |
| Agent 历史会话 | Aarre | 当前 Chrome 配置文件本地 |
| AI 模型密钥 | 当前 Chrome 配置文件 | 由扩展直接发送给用户选择的 AI 服务商 |
| 待同步内容 | 当前 Chrome 配置文件本地 | 当前不发送；等待 F14 自建同步服务 |

统一输入框只在用户输入时查询 Chrome History API 生成本机联想，不会把完整浏览历史写入产品数据库。Chrome 星标和 Aarre 保存都固定创建完整增强任务，不再提供容易造成半成品的 AI 勾选项。权限、AI Key、网络或合适的前台页面暂不可用时，原生书签照常保存，AI 与截图任务持久等待，并在配置恢复或该收藏下次以任意正常方式打开后继续。无痕、内部、局域网、银行、支付、医疗和用户排除页面不读取正文、不发送给 AI、也不截图；页面快照只保存在本机。当前构建未启用云端同步，不会把这些内容发送到 Aarre 自己的服务器。
