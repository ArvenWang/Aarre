...
- 存量缺图收藏可在收藏库中显式启动「补齐缺失封面」：Aarre 复用一个专用前台标签页逐个加载和截图，失焦即等待，并支持暂停、继续和取消。该任务只生成本机截图，不调用 AI、不上传图片，也不会申请高风险调试权限。
...
- `captureVisibleTab()` 只能截当前活动标签页；连续打开多个缺图网站后，先退到后台的页面不会在后台直接截图。它们会保留待办，并在用户之后切回且页面稳定时自动补拍；需要一次处理存量缺图收藏时，使用收藏库里的显式前台补拍任务。
