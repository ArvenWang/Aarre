...
最后更新：2026-08-03（0.5.40 / 书签标题显示账号名可跳转设置，右键图片可设为封面）
...
**当前工作区最新状态：0.5.40。** 0.5.40 新增两项能力：侧边栏“我的书签”在登录后显示“用户名 的书签”（如 `Arven wang (Nefish) 的书签`，超长截断），点击标题直接打开设置页的账号与同步区；网页右键菜单新增“用此图片设为封面”（`image` 上下文）——点击后校验页面已收藏与隐私保护规则，下载图片（≤15 MB、http/https）经 `createImageBitmap + OffscreenCanvas` 缩放到最长边 1600px 转 WebP，写入资源封面并立即触发云端同步；原有“更新封面”（整页截图）保持不变。0.5.39 起完整备份会循环把云端图片全部恢复到本机（进度实时到 391/391）；0.5.38 构建门强制云端；服务器 active 图片 391 张（原 409 的 95.6%）。0.5.37 上传前对账；0.5.36 只有完整备份；0.5.35 manifest 固定扩展 ID `ppjmhonejgpcdmjmcbbdjookgiagambm`。当前 `dist/` 为 cloud-enabled 0.5.40 正式构建（带 key）。F14 生产 API、数据库、COS/CAM、Google Web OAuth、DNS/TLS、定时备份、两分钟健康巡检、独立 GlitchTip project 与含 SSH Key 的加密恢复包已经部署；Google 品牌审核、卸载重装恢复和正式 Web Store ID 尚未完成。
