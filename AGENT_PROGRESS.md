...
**Aarre 0.3.6 已完成侧边栏交互与设置页的产品级收口，并纳入整改复核阶段的矢量图标修复。** 悬浮预览、编辑浮层、文件夹扁平样式、搜索焦点、页面切换滚动条和设置页排版均已在 320px / 390px 两档真实浏览器环境回归。第 4 批的分类封面兜底率仍未达标：经修复矢量图标闸门后从 59.67% 降至 **51%**，距 PRD 的 ≤12% 硬指标仍有 39 个百分点。后续对照实验**修正了此前的归因**：主因不是「闸门太严」（调低尺寸门槛实测零效果），而是 43% 的样本站点根本产生不了任何候选，外加墨迹闸门把分母算成整张画布、使尺寸闸门沦为死代码。可做的两项修复已写入 `docs/REMEDIATION.md` 第 7 批，不在本次侧边栏 UI 修复中扩张范围。M1 的 F1–F6 / F22、M2 的 F7–F13、M3 本地项 F15–F17、M4 本地项 F18–F20 均已有正式实现；F8 按 PRD 第 12 章只实现已决策的第一层强词法检索。按用户要求，F14 自建云端与依赖云端的 F21 Web/PWA 不在本轮范围，现有云端文件未顺手改动。
...
**下一轮要做的事全部写在 `docs/REMEDIATION.md`（已更新到 v2）。** 第 1–6 批已交付并通过复核，**接手的 Agent 只需执行第 7 批**（F3 收尾：补 `/favicon.ico` 资产来源、修墨迹闸门算错的分母）。**开工前必须先读 7.0**——那一节记录了已经被实验证伪的路径，避免重走。不要自行另立计划。
