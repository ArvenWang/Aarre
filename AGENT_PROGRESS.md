...
**Aarre 0.3.1 已完成本轮约定的全部云端外代码与本地交付物。** M1 的 F1–F6 / F22、M2 的 F7–F13、M3 本地项 F15–F17、M4 本地项 F18–F20 均已有正式实现；F8 按 PRD 第 12 章只实现已决策的第一层强词法检索。按用户要求，F14 自建云端与依赖云端的 F21 Web/PWA 不在本轮范围，现有云端文件未顺手改动。
...
1. 在允许访问 `chrome://extensions` 的真人 Chrome 中加载 0.3.1 `dist/`，按本页“明确未通过 / 外部门”逐项补证；涉及删除时先建立专用测试文件夹，不在真实私人书签上冒险。
...
- `npm run check`：通过；包含 TypeScript 类型检查、28 个测试文件 / 98 项测试和生产构建，`dist/manifest.json` 版本为 0.3.1。
...
- 尚未声称最新 0.3.1 `dist/` 已在 Chrome 扩展管理页加载；本轮内部页访问被浏览器安全策略阻止。
