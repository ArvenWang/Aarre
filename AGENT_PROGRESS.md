...
**网页端收藏库已完成 0.4.4 稳定性收口。** Aarre 品牌与六个 Tab 合并为一条头部，只保留一条整体分隔线；收藏搜索、状态、文件夹和排序维持收藏库自身工具栏。瀑布流悬停详情改为不参与 CSS 多栏排版的绝对覆盖层，封面和卡片占位不再改变，已消除第二列反复重排闪烁。每张卡片仍可精确编辑或删除一个 Chrome 收藏位置；大封面只读取本机 `pageSnapshots`，缺图时在 40 张 Aarre 兜底图中稳定分配。
