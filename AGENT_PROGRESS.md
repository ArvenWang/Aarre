...
**Aarre 0.3.2 已完成本轮约定的全部云端外代码与本地交付物。** M1 的 F1–F6 / F22、M2 的 F7–F13、M3 本地项 F15–F17、M4 本地项 F18–F20 均已有正式实现；F8 按 PRD 第 12 章只实现已决策的第一层强词法检索。按用户要求，F14 自建云端与依赖云端的 F21 Web/PWA 不在本轮范围，现有云端文件未顺手改动。
