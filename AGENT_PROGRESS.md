...
Chrome 原生书签增强产品已完成 0.2.3 可靠性加固。侧边栏读取 Chrome 的全部原生书签根目录；右键菜单会携带页面或链接上下文自动打开侧边栏编辑表单，用户确认后再创建真实书签。离线队列、重复收藏、危险删除、键盘焦点和交付物版本校验已加强。Google OAuth、Supabase 和 Gemini 代码已实现，但真实服务尚无项目凭据，不能宣称上线完成。
...
- 正式品牌名、图标和商店素材尚未确定。
- 云端 Gemini 调用尚未建立正式的按用户限流和费用配额。

## 验证情况

- `npm run typecheck`：通过。
- `npm run test`：7 个测试文件、23 个测试通过。
