...
最后更新：2026-09-09（依据 14 条用户批注实施 0.6.2：单一工作区与底部输入框）

## 当前进行 · 0.6.2 按用户批注重整菜单

用户对 0.6.1 明确提出 14 条批注，**上一轮“视觉通过”只代表当时的检查记录，不代表用户已认可**。本轮从干净提交 `8b9a1e3` 开始，继续沿当前分支实施，不推送或发布。

- 产品结构：移除悬浮菜单的收藏/AI/设置 Tab。收藏与对话在同一工作区，底部常驻较大的输入框；设置、历史为次级入口，不再提供独立 AI 欢迎页。“收藏当前网页”改为头部安静的图标操作。
- 共享视觉：清除搜索、筛选、按钮和标签上的遗留胶囊；清除输入框双层边框/阴影；控件内容垂直居中，字段标签另行且保持 8px 间隔，箭头 12px 内边距；选中/鼠标聚焦不使用粗描边，键盘焦点保留细且可辨认的提示；日夜开关的拇指均为亮色。
- 布局：收紧设置外围与章节留白；窄窗主题操作归入固定工具组；删除确认改为上方说明、下方同一行按钮，禁止一侧竖排按钮挤压说明。
- 可验证目标：打开菜单即可输入；输入前后同一个 composer DOM 和草稿持续；无 AI/设置导航 Tab；未配置时保留输入并引导到真实设置；流式回答、停止、历史、重试、操作确认继续走现有正式业务。320/400/560px 明暗主题下，正文无横溢出，只有表格/代码块自身可横滚，输入区和操作按钮始终可达。
- 验收逐项对应批注 1～14，实际运行产品组件、浏览器操作、量测和逐图审核并行。新图集先显示最新界面，修复前图片明确收起，不再混在当前画面中；回归测试以真实用户路径为主，完整检查与 0.6.2 独立封装在最后执行。
- 已完成代码与当前可复查结果：单一工具栏/工作区与常驻输入、辅助窗口、共享小圆角/单层搜索/单线焦点、紧凑设置、两行删除确认、亮色开关；额外修复流式内容已返回却不显示的问题，并让提案摘要显示具体操作。宿主慢加载曾超时但后来恢复时，现在会清除重试状态，避免下一次展开重建菜单并打断会话；修复前回归失败、修复后 2 个宿主用例及 25 个既有用例通过。
- 已执行验证：17 组菜单和 21 组管理页量测无横溢出或缺失自制滚动条；320/400/560px 长回答仅在块内横滚；真实发送/停止/继续/历史/新会话回放通过，输入元素与草稿连续。4 个根应用集成用例和相关约束共 44 项专项通过，TypeScript 通过。当前已逐图检查 61 张画面；实际宿主重开后保留同一 iframe / composer / 中文草稿、恢复输入焦点、原网页宽度不变，明暗主题及 420px 窄窗均无越界。正式完整检查与封装正在收尾。
- 当前证据入口：[0.6.2 图集](docs/verification/2026-09-09-conversation-workspace/index.html)、[14 条批注对账](docs/verification/2026-09-09-conversation-workspace/README.md)。旧图集已改为折叠历史页并引导到本轮；早期未完整绘制封面的截图不计入通过，已通过实际主题按钮和图片解码等待补拍。
- 当前占用：`src/ui/floating/`、`src/ui/sidepanel/`、共享 UI/样式、管理页控件、验证用例与进展文档。请其他任务不要同时修改。
