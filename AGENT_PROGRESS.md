...
Chrome 原生书签增强产品已完成 0.2.3 可靠性加固。M1 的 F1–F6、F22、F8 第一层检索，以及 M2 的 F7、F9–F13 已完成代码和自动化阶段；真实安装扩展验收门仍统一保留到最终收口。下一开发重点是云端以外的 M3 / M4 本地功能和最终真实 Chrome 验收。
...
1. **M2 差异化能力**：F7 主动整理提案 → F9 死链检测 → F10 重复合并 → F11 扫描并发、限速与成本透明 → F12 保存时文件夹建议 → F13 待读队列。
2. **非云端上架项**：F15 隐私与合规、F16 计费与配额接口的本地/契约部分、F17 正式商店素材。
3. **M4 本地能力**：F18 周报月报、F19 主题图谱、F20 遗忘曲线重现。
4. **最终统一验收**：加载当前 `dist/`，逐条补齐 F1/F2/F3/F4/F5/F6/F8/F22 的真实 Chrome 扩展门；随后做全量构建、安全、隐私、产物和文档审计。
5. **按用户要求排除**：F14 自建云端与依赖云端运行的 F21 Web/PWA 不在本轮实现范围；现有云端代码不顺手改动。
...
- `npm run test`：19 个测试文件、78 个测试通过。
