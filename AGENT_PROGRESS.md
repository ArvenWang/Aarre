## 三仓视频稳定性修复配套交付（2026-09-20）

- 用户明确要求检查三个项目现有改动，分别提交并推送。本仓入场 main `e45fc18a`，已拉取远端且与 origin/main 一致；工作区只有下方此前保留的 2026-09-17 主线同步/本机构建进展。本次经审查一并纳入文档交付，不再沿用历史批次的排除范围。
- 本轮产品修复在 NexCatcher `2b4b582`：取消列表阶段逐条样本解码，预检和预览使用可终止的 Worker，限制并行工作、读取及缓冲，预览/隐藏标签页暂停后续检查。证据见兄弟仓库 `docs/execution/VIDEO_RESOURCE_STABILITY_2026-09-20.md`；不把其测试当作本项目全部业务验收。
- 三仓 9 个 `src/shared/suite-dock/` 模块重新逐字节核对一致。本仓无新增产品代码、配置或构建产物改动，只有进展文档；未重复构建或运行收藏/云端业务测试，历史构建成绩保持原日期。
- 已完成本次文档审查与差异格式检查，按用户授权单独提交推送；最终提交及远端核验为准。日常 Chrome 本批未重载/人工验收，长期运行与用户自然崩溃不冒充已验证。无持续文件占用，无子代理。

## 常驻入口与瀑布流稳定性修复（2026-09-19）

- 按用户三款每页稳定常驻、点击后按需载入的授权实施。同步健康 PING 不再重新初始化；同版本且健康的宿主不重复注入替换；只有 BFCache 恢复才重新核验会话。
- 共享连接短时中断保留原入口，合并首次成员，隐藏页暂停驱逐，恢复前台后继续识别已退出成员；断连时本地点击立即展开，跨工具点击待恢复后只转发一次。三仓 9 个 suite-dock 文件逐字节一致。
- 本轮类型/构建和 40 个 JS 产物语法检查通过，定向 5 文件/56 项通过。早期误包含 work 历史副本、旧 PING/BFCache 契约失败，以及断连本地点击缺陷已分开记录和修正。最终三款隔离浏览器切换 8 次保持原宿主；双工具 BFCache/重载/重装 12 阶段通过，无 runtimeErrors。
- 日常 Chrome 未重载/人工验收，全部收藏业务、长期内存与真实 Pinterest 自然崩溃未验证。跨仓证据在 NexCatcher 的 docs/execution/FEED_VIDEO_RESIDENT_DOCK_2026-09-19.md 与 reports/videos/feed-resident/verification.json。
- 本批仅交付相关源码/测试/本段进展；入场前未提交进展完整保留，不纳入提交。dist 已更新，Git 以本批提交及远端核验为准；无持续文件占用、无子代理。

## 三款长页面稳定性实施（2026-09-19）

- 用户授权 NexCatcher 稳定性评估落地，允许同步修复三款共享入口。本批保留独立项目、收藏业务与既有草稿，不合包，无子代理。
- 共享入口先打开目标工具，旧工具退出交互后异步保存；新意图优先、迟到回复不覆盖新打开状态，外部成员忙碌宽限为 15 秒。与 Catcher/NexAlign 的 9 个共享模块逐字节一致。
- Aarre 宿主初始化连接失败保留入口；收起立即退出交互，保存成功且未重开才卸载 iframe，失败保留草稿。拖宽每帧处理最新宽度；正文提取约 5ms 分片复制可读节点，排除脚本/SVG/UI，保留来源元信息、选择文本与原输出上限。
- 验证分轮：定向 5 文件 46 项、补充宿主 34 项、提取 2 项分别通过；类型、本机构建及 40 个输出 JS 语法检查通过。Catcher 仓库一万节点八种组合 8/8；Aarre 单/双/三装拖宽 P95 约 33.3–33.4ms。原始误匹配归档测试、旧心跳断言与测量节奏失败均已单独记录，不计为全量业务验收。
- 日常 Chrome 未重载/验收，长期内存及真实 Pinterest 自然崩溃未验证。跨仓完整记录见 NexCatcher 的 `docs/execution/SUITE_STABILITY_IMPLEMENTATION_2026-09-19.md` 与 `reports/suite/stability-implementation-2026-09-19.json`。
- 本批只交付上述源码、相关测试与本段进展；入场前未提交进展完整保留，不纳入本次提交。dist 已构建，Git 交付以本批提交/远端核验为准。无持续文件占用。
...
# 本次主线同步与本机构建（2026-09-17 21:36）

- 按用户要求与 NexAlign 一起拉取最新主线。开始前工作区干净，`git pull --ff-only` 返回已是最新，Aarre `main` 为 **`fded72e8`**，与 `origin/main` 差异为 0。NexAlign 已从 `e1d563d` 快进到 **`fa4f38a8`**，产品版本更新为 **0.2.0**；下方与 0.1.3 联动的记录是前次交付。
- 本轮完成 `npm run build`，沿用项目现有生产云端配置；本机 `dist/` 已更新，源码、public Manifest 与 dist Manifest 均为 **0.6.32**。构建内置的 **40 个 JS 语法检查通过**。
- 构建有两项非阻断提示：CSS 压缩器提示 `@theme` 为未知规则；`visuals.ts` 同时被静态/动态导入，无法按该动态入口独立拆包。两者没有阻止本次构建；本轮只同步和构建，没有顺带修改产品样式或拆包逻辑，也未据此声称运行体验已验收。
- 与 NexAlign 的 **9 个 suite-dock 模块逐字节一致**。NexAlign 正式构建及 **21 个 JS** 发布产物检查通过，构建标识为 **`0.2.0 · fa4f38a8 · 20260917T1336Z`**。
- 本轮未运行全量测试、浏览器回归、AI 或云同步验收，未重载日常 Chrome；用户重载两只扩展并刷新网页后使用新包。只更新本机构建与本进展记录，未提交或推送；无源码文件占用。

# 前次交付：0.6.32 与 NexAlign 0.1.3 联动及双仓库 Git 交付（2026-09-17）

> **没有经过人工验证。** 以下是自动化检查和代理工具操作记录，不代表用户人工验收；两个扩展的最终使用体验仍待用户复验。

- **交付范围：** 按用户要求，把本会话 Aarre 的左右拖动/就近贴边、对应菜单和收藏表单换边、重载/BFCache 恢复、手机模式按标签页隐藏整套悬浮入口，以及对应测试和进展一并交付到 `main`。版本 **0.6.32**，配套 NexAlign **0.1.3**；下面历史记录中的“未提交/推送”仅指当时状态。未新增权限，不发布商店。
- **联动状态：** 两仓库九个 `suite-dock` 模块逐字节一致。进入手机检查模式后仅隐藏对应页的 Aarre，退出/恢复桌面自动显示，其他桌面页不受影响；共享左右位置及原有偏好继续保留。手机状态由 NexAlign 后台真实设备会话发布，异常断连有回收，不保存永久隐藏状态。
- **NexAlign 最新配套修复：** 用户反馈的紫色整体分栏原走 Chrome 原生 Overlay，存在额外延迟且会压在浮动侧栏前面。NexAlign 已改为分栏与轮廓同帧绘制，统一裁切左右侧栏。指定本地搜狗页及 7 种布局的最终检查 **22/22**，12 次完整分栏响应中位 **10.7ms**、最大 **16.8ms**；悬停回归 **25/25**，暂停清理 **30/30**。本次分栏修复无需改 Aarre 产品代码。
- **有效验证与版本边界：** Aarre 产品源码自上一轮手机联动最终验证后未变，沿用类型、设计边界、正式构建、40 个 JS 语法及交付物检查、3 文件 **42 项**定向测试，以及 Chrome 152 双扩展手机联动 **14/14** 的有效证据。更早左右吸附的 Chrome 152 **71/71** 和重载 **12 阶段**证据保留在下方，不冒称本次重跑；未重新验收全部收藏业务、在线 AI 或云同步。两仓库 package / lock / Manifest / dist 版本已核对。
- **提交后产物与安装：** Aarre 产品提交 `06e7a02` 后重新构建，40 个 JS 语法和交付物检查通过；配套 NexAlign 源码提交 `e4671b18` 的正式构建与发布产物门通过。已通过工具在日常 Chrome 重载两只扩展，管理页显示 Aarre **0.6.32** 和 NexAlign **`0.1.3 · e4671b18 · 20260916T1927Z`**，没有两者的错误入口。用户本地搜狗页刷新后双入口正常，打开 NexAlign 并选中整栏，分栏没有覆盖右侧栏。此为工具操作，**没有经过人工验证**。
- **证据与后续：** 本地 `work/grid-overlay-20260917/` 保存交付前备份；本次分栏报告在兄弟 NexAlign 仓库同名目录，Aarre 本体证据在 `work/hover-mobile-20260917/` 和 `work/dock-selection-reload-20260917/`。备份、截图、大型验证产物和 dist 不入 Git。待用户人工复验左右贴边、手机隐藏/桌面恢复，以及 NexAlign 分栏跟手和侧栏避让。

# 前次交付：手机检查模式隐藏悬浮入口 0.6.32（2026-09-17）

- **用户范围：** NexAlign 切手机时，Aarre 不再挡在小尺寸网页侧面；恢复桌面后入口自动出现，其他标签页不受影响。同时配合 NexAlign 的悬停延迟修复。
- **实现：** 两仓库新增相同的 `suite-dock/viewport.ts`，以扩展后台之间的连接传递当前手机标签页。Aarre 的目录组按标签页接收临时 `suppressed` 状态，整个悬浮宿主及子项隐藏、禁止点击/键盘交互并收起快捷操作。桌面或退出时恢复；网页刷新、导航及 Aarre 重载重新接入。没有修改用户原来的悬浮开关、左右边缘和纵向位置偏好，单独安装/其他桌面标签页照常使用。
- **稳定性：** 手机状态由 NexAlign 实际设备会话产生，允许列表、扩展来源和后台身份均校验，网页或内容脚本不能跨页发出隐藏命令。状态只随后台连接存活，正常退出即释放，异常断连/无响应有回收；没有持久化隐藏标记或增加权限。九个共享模块逐字节一致，两边 `docs/shared-dock.md` 已更新。
- **验证：** 类型、设计边界、正式构建、40 个 JS 语法及交付物检查通过，3 文件 **42 项**定向测试通过，覆盖手机状态按页隔离、换文档、恢复、错误来源/无效消息及失效连接。可见 Chrome 152 的真实双扩展检查 **14/14**：两次手机/桌面往返、其他桌面标签页仍可用、手机页刷新、实际重载 Aarre 后继续隐藏；Chrome 自身的双方 runtime/manifest 错误列表为空。未改收藏业务、操作用户收藏或调用 AI/云同步。
- **NexAlign 配套进展：** 悬停期间延后普通目录快照，同一元素不重复刷新侧栏，并修复旧点击完成后清除新悬停的竞态。保持目录过滤和 600 项上限，点选缺项立即补齐。最终搜狗本地结果页 33 次悬停 P95 **19.4ms**，11 个点选元素均能定位；密集动态页 20 次最大 **26.8ms**，悬停/目录回归分别 **25/25**、**10/10**。这些性能样本来自真实扩展上下文的 Chrome for Testing 148，不能外推所有网站。
- **版本与日常 Chrome：** package / lock / public Manifest / dist 均为 **0.6.32**；NexAlign 为 **`0.1.2 · 94c3aeb2 · 20260916T1847Z`**。已实际重载两只扩展并核对版本；用户本地搜狗页切手机后 Aarre 消失，返回桌面并收起 NexAlign 后配对入口恢复。管理页没有新增错误入口，本轮没有清空错误记录；测试页已还原桌面状态。
- **证据与交付：** 本地备份、类型/构建日志在 `work/hover-mobile-20260917/`；双扩展最终报告、截图及 Chrome 错误列表在兄弟 NexAlign 仓库同名目录的 `mobile-final/`，其他最终性能证据见其 `AGENT_PROGRESS.md`。本轮完成，无文件占用；保留所有先前未提交工作，未提交、推送或发布商店。未重跑无关全量业务回归。

# 前次交付：左右吸附与重载恢复 0.6.31（2026-09-17）

- **用户范围：** 与 NexAlign 联动支持左右及上下拖动，松手吸附最近边缘，并排查 Chrome 重载错误、开始版本迭代。
- **实现：** 两边 `src/shared/suite-dock/` 八个模块逐字节一致。拖动期间跟随双轴、松手按中心所在半区吸附、正中保留原边；持久化 `side` 与 `handleRatio`，旧设置默认靠右。共享位置由 Aarre 持有并通知 NexAlign。左右主菜单、快捷收藏、收藏表单、改宽把手及反馈位置同步换边，单独安装继续可用。左右方向键换边，上下/Home/End 调整高度，Tab 进入收藏。统一修复配对手柄贴底后反向移动的空行程，保留入口加入时锚定首槽的规则。
- **重载恢复：** 日常 Chrome 实際读到 Aarre 的 BFCache 正常断连未处理错误；同步 NexAlign 后台 `lastError` 处理和已安装 Manifest 允许列表，未增加权限。内容端对失效上下文消息及时清理；断连错误的读取使用发布压缩后仍保留副作用的 `Reflect.get`，不做全局异常屏蔽。NexAlign 同步修复旧回调访问 `runtime.id`，并修复点选元素被目录简化/600 项上限遗漏的问题。
- **验证：** 类型、设计边界、正式构建、40 个 JS 语法及交付物检查通过；7 文件 **91 项**定向测试通过。可见 Chrome 152 真实双扩展 **71/71，0 页面异常**，覆盖左右中途松手、拖动跟手、左侧菜单/收藏/改宽/图层定位、刷新、窄窗、上下及左右键、Tab 收藏、减少动态效果、真实收藏写入和两边单独安装；已目视截图。Chrome 自身错误列表在 NexAlign 的 **12 阶段**缓存往返、重复重载及重装验证中均为空。未重跑与本轮无关的全部 625 项测试，NexAlign 目录验证为 10/10。
- **版本与安装：** package / lock / public Manifest / dist 均为 **0.6.31**；NexAlign 为 **`0.1.1 · 94c3aeb2 · 20260916T1747Z`**。已在日常 Chrome 重载两只扩展并核对版本；清除旧记录后没有新增错误入口。搜狗公网首页实际拖动共享入口到中途松手吸附左侧，并从左侧打开 NexAlign、点选元素，目录对应行可见。未操作日常 Chrome 的真实收藏数据或在线 AI/云同步。
- **证据与交付：** `work/dock-selection-reload-20260917/` 保存备份、日志及 `browser-final-3/` 最终截图/71 项报告；NexAlign 同名目录包含基线、目录和 `reload-final-3/` 报告。早期失败包含真实底边比例问题及动画尚未结束的观察时序，均已修正后复测。`docs/shared-dock.md` 两边同步。当前本轮完成，无文件占用；未提交、推送或发布商店，既有未提交工作保留。

# 本机构建与安装进展（2026-09-16）

- 按用户要求，拉取后的 main `765c1e4` 已构建为 0.6.30，输出至原安装目录 `dist/`，沿用项目云端构建配置。
- 本轮通过类型检查、设计 token 检查、正式构建、40 个 JS 语法检查及现有交付物检查。日志在 `work/build-install-20260916/`，未重跑全量测试或浏览器回归。
- 已只读确认日常 Chrome 的 Aarre 安装路径为本仓库 `dist/`，固定安装 ID 保持。用户随后确认已自行重载；09-17 日常 Chrome 扩展页显示 Aarre 0.6.30 且启用，并在 `https://sogou.com/ihome` 看到了 Aarre / NexAlign 共享入口，单击 NexAlign 可正常打开。本轮未重新验收收藏写入、AI 或云同步。
- 与 NexAlign 八个共享模块中七个一致，`background.ts` 尚缺对方已有的断连原因处理和 Manifest 允许列表读取；本轮按现有主线构建，未改源码。当前 NexAlign 安装 ID 已在本构建允许列表中。
- 未提交或推送；本轮只更新构建产物和此进展，无占用文件。

# 本次主线交付：共享侧栏阶段进展 0.6.30（2026-09-15）

- **同步范围：** 按用户要求，与 NexAlign 一并整理并同步自上次主线交付后的共享侧栏进展：距右边 8px、全圆角、仅上下拖动及持久化、52×96px 品牌入口、图标间距/悬停/收藏状态优化，以及与 NexAlign 一致的开合和实际右边缘平移动画。收藏任务保持独立尺寸并避让上下边缘；共享源码包仍仅为评估。
- **本轮验证：** Node、设计边界、类型、**110 个文件 / 625 项全量测试**与正式构建通过，40 个 JS 产物语法检查通过；package / lock / public Manifest / dist 均为 0.6.30。全量检查发现旧收藏收起测试在 jsdom 中读到 0 宽度，现仅补上浏览器尺寸测量边界，继续断言收起过程保留原表单尺寸及生命周期；未因此修改产品行为。
- **联动与证据：** 两边八个共享模块逐字节一致。此前实际双插件动画、拖动、主题、收藏、单独安装等验收证据沿用下方完成记录。本轮 NexAlign 另完成 Figma 6:4 布局及相似图片对照，真实扩展检查 95 项通过，含实际手机原生侧栏保持；该插图不涉及 Aarre。
- **Git 与交付：** 本提交保存上述配套源码、测试、共享维护评估与进展记录；同步前 main 与 origin/main 无分叉。本轮备份及日志在 `work/figma-picture-sync-20260915/`，最终全量日志为 `check-final.log`。本机 dist 已更新，工作截图和临时资料不入库；未重载日常 Chrome 或发布商店。

# 已完成：共享右边缘平移动画 0.6.30（2026-09-15，本机）

- **用户本次调整：** 取消上一轮保留空滚动条凹槽的方案。菜单始终按当前可用区域保持右侧 8px 距离；有滚动条宽度释放时，随展开一起自然向右平移，关闭时平滑返回。没有宽度变化时不增加横向动作。保留上一轮空态提示 40% 定位及此前图标/胶囊修改。
- **实现：** 移除空槽预留、宽度扣除与空槽滚轮拦截；`dockViewport()` 返回当前真实可用视口。NexAlign 新增预载 `floating-host.css`，仅匹配已展开的 PC 宿主，点击当帧隐藏滚动条，使展开直接使用最终宽度；慢后台不会导致随后再移动。Manifest 和旧标签页手动注入均装载该样式，宿主关闭/移除后规则失效，手机宿主不匹配。旧凹槽标记仅留升级/强制清理兼容。
- **共享动画：** 两边 `morph.ts` 同步修改；只缩放中性底板，内容与关闭按钮/把手按实际右边缘变化同步平移，保留 280/210ms 开合、140/120ms 淡入淡出及时序。视口变化从当前显示位置续接、不延长正在进行的动画截止时间；Aarre 收起时保持原表单尺寸并对齐新的右边缘。八个共享模块逐字节一致。
- **真实扩展验证：** 最终可见 Chrome 152 的定向检查 **65 项通过**（普通 18px / 细 11px / 无滚动条 / 人为延迟真实后台连接），另有联动回归 **50 项通过**，均无记录到的运行异常。普通场景右边缘从 1254→1272px，展开和视口变化同一帧开始，10 个中间帧，内容/底板误差约 0.00012px、无越界回弹；细滚动条平移 11px，无滚动条位移 0。延迟场景画布激活比展开晚约 1001ms，横移仍在最初的 280ms 动画内完成。关闭、实际缩放/复位、日夜、上中下拖动、收藏、相同动画、减少动态效果、单插件安装均覆盖。已目视检查实际截图。
- **设备与工程：** 同一最终构建完成手机往返 **39 项通过、0 异常**；动画/几何测试 **33 项通过**。双方类型和 UI/设计边界、正式构建、NexAlign 25 个 JS 等产物与包体检查、Aarre 40 个 JS 语法检查通过。NexAlign `0.1.0 · 5b81faf3 · 20260914T1626Z`（UTC 构建标识），Aarre `0.6.30`（package / lock / Manifest / dist 一致）；两边 dist 已更新。SourceCanvas gzip 36293 / 36545 B、background 119672 / 120066 B，未提高包体上限。
- **证据与过程：** 两仓库 `work/dock-edge-motion-20260915/` 保存本轮备份及工程日志。Aarre `first/`、`thin/`、`none/`、`slow/`、`regression-final/` 为最终桌面证据；NexAlign `mobile-final/` 为最终手机证据。早期回归的收藏悬停读取与桌面缩放后点击遇到输入时序问题：验证改为等待实际鼠标悬停反馈、对 iframe 按钮使用带位置稳定检查的真实点击；未修改产品来绕过断言。原始失败报告保留，未计入通过结果。可复用双插件脚本同步该等待逻辑。
- **交付状态：** [实际展开/延迟场景录屏](/Users/nefish/Desktop/Coding/Aarre/work/dock-edge-motion-20260915/review.html)。`docs/shared-dock.md` 已同步新方案，以下旧空凹槽记录作为历史保留。本轮完成，无文件占用；其他未提交工作保留。未重载日常 Chrome、提交/推送或发布商店；重新加载两只扩展并刷新网页后生效。

# 已完成：统一原位淡入与空凹槽定位 0.6.29（2026-09-14，本机）

- **定位与修复：** 在独立可见 Chrome 152 复现：内容入场的 `translateX(12px)` 造成 Aarre / NexAlign 最大约 9.72 / 9.61px 向右探出；原生滚动条消失另使 NexAlign 菜单右移 18px。两边共享动画改为内容原位淡入/淡出，底板仍沿固定右边缘展开，保持 280/210ms 和 140/120ms 的原时序。八个共享模块逐字节一致。
- **空凹槽：** NexAlign 在 PC 激活前记录原滚动条实际宽度，以 `overflow: hidden` + `scrollbar-gutter: stable` 留出无滑块的空槽；共享 `dockViewport()` 补偿空槽仍计入 VisualViewport 的差异。两个菜单、把手和画布滚动控件保持原右边缘；点击、拖动及滚轮均不会操作空槽。无原始凹槽、覆盖式滚动条和手机模式不额外占位；关闭/暂停/异常清理时恢复页面原状态。
- **空态布局：** NexAlign PC 检查器的空提示中心位于上半区约 40%，按区域百分比定位；图层区增高、还原、最小上半区高度均自适应并完整显示。加载态及手机布局保持。
- **真实扩展验证：** 最终构建在独立可见 Chrome 152 共 **99 项通过、0 记录到的运行异常**：普通 18px 凹槽 17 项、细 11px 凹槽 17 项、无槽 15 项、既有联动回归 50 项。逐帧两菜单内容向右探出均为 0px，右边缘全程固定；覆盖空槽无操作、页面正常滚动、缩放/复位、关闭恢复、提示比例、日夜、拖动、收藏、相同动画、单独安装、键盘和减少动态效果。已目视检查实际截图。原有动画/几何定向测试 31 项通过；更新已有动画断言为内容原位渐变。
- **工程与构建：** NexAlign `0.1.0 · 5b81faf3 · 20260914T1548Z`，Aarre `0.6.29`（package / lock / Manifest / dist 一致）。两项目类型及 UI/设计边界、正式构建和产物检查通过；NexAlign SourceCanvas gzip 36317 / 36545 B、background 119693 / 120066 B，未提高包体上限。两边 dist 已更新。
- **手机证据：** 本轮 NexAlign `1538Z` 构建在真实扩展上下文完成手机往返 39 项、0 异常，包含原胶囊/分隔线/标题与设备往返。其后仅精调 PC 空态最小高度及 PC 保留空槽上的滚轮拦截，复用该手机证据，不冒称在 `1548Z` 重跑。
- **过程与证据：** 两仓库 `work/dock-motion-gutter-20260914/` 保留备份与工程日志；Aarre 的 `baseline/` 为修复前逐帧记录，最终结果为 `final-classic/`、`final-short-2/`、`final-thin/`、`regression-2/`。早期空槽滚轮仍平移画布的问题已修复；观察脚本的调试连接/DOM 节点失效和原生窗口激活竞争以有限重试处理，早期失败不计入通过结果。测试备份改用 `.bak`，避免被测试收集器误识别。手机报告在 NexAlign 同名目录的 `mobile/`。共享规范 `docs/shared-dock.md` 已同步。
- **交付状态：** [实际录屏与截图对照](/Users/nefish/Desktop/Coding/Aarre/work/dock-motion-gutter-20260914/review.html)。本轮完成，无文件占用及待办；此前未提交修改保留。未重载日常 Chrome、提交/推送或发布商店；重新加载两只扩展并刷新网页后生效。

# 已完成：紧凑共享图标间距 0.6.28（2026-09-14，本机）

- **本项目：** 与 NexAlign 同步共享 `ui.ts`。两个 36px 正式图标之间、图标与外框均为 8px；44px 点击区相邻，外 padding 4px，共享把手 **52 × 96px**，独立仍为 52 × 52px。已批准的图标、轻量 hover、无分隔线、单层收藏入口、上下拖动、主题及两主菜单相同动画保持。未修改手机业务。
- **关联 NexAlign：** PC 底部三组胶囊改为 4px 留白/R10，删除底部叠加横线、检查空态顶部“选择页面元素”和 Figma“设计对比”抬头；保留 Figma 导入后的操作按钮。只作用于 PC 浮动菜单。
- **工程与构建：** package / lock / public Manifest / dist 均为 **0.6.28**；类型、设计边界、正式构建、40 个 JS 语法及版本一致性通过。NexAlign 正式构建 `0.1.0 · 5b81faf3 · 20260914T1501Z` 通过类型/UI 边界、产物与包体门。两边八个共享模块逐字节一致。
- **真实扩展：** Chrome 152 独立可见窗口中 **65 项通过，0 记录到的运行异常**，包含本次可见间距/胶囊几何/实际无横线/两处抬头/Figma 导入移除、明暗与窄窗，以及拖动、相同动画、收藏与双方单安装回退。最终截图已目视检查；可复用把手脚本已改为检查图标间距而非按钮间距，并容忍菜单关闭时只读观察遇到调试器正常释放。NexAlign 手机 39 项在 `1459Z` 通过；最后的 PC 边框覆盖修正不影响该路径，证据边界见其进展。
- **交付：** 备份与日志在两仓库 `work/dock-panel-spacing-20260914/`，最终报告/截图在本项目 `final-2/`，[实际截图对照](/Users/nefish/Desktop/Coding/Aarre/work/dock-panel-spacing-20260914/review.html)。既有未提交进展保留；本轮完成，无文件占用。未重载日常 Chrome、提交/推送/发布；重新加载两只扩展并刷新网页后验收。

# 已完成：恢复应用图标与简化悬停层级 0.6.27（2026-09-14，本机）

- **一级入口：** 恢复 Aarre / NexAlign 正式 SVG 品牌图标，36px / R8，按钮仍为 44px。两个入口的间隔增为 8px，去掉横线，共享把手为 52 × 104px；保留 8px 离边、四角 16px、纵向拖动和两菜单相同开合实现。独立安装仍为 52 × 52px，显示自己的正式图标。
- **悬停与收藏：** 主入口使用轻微底色反馈。左侧收藏功能删除外壳边框、内部选中框/底色及小状态点；内部按钮始终透明，hover / 按下只改变一层外壳底色。已收藏用实心星标表达，普通状态为描边星标；键盘 focus-visible 保留细轮廓，原真实收藏/管理/取消流程保持。
- **构建与验证：** package / lock / Manifest / dist 均为 **0.6.27**；类型、设计检查、正式构建、40 个 JS 语法和版本一致性通过。NexAlign 同步正式构建、类型/UI 边界、25 个 JS 等产物及包体门通过；八个共享模块逐字节一致。本轮为低风险外观修订，复用真实扩展定向验证，没有新增单元测试或重复无关全量回归。
- **真实 Chrome：** 独立可见 Chrome 152 中 **50 项全部通过，0 记录到的运行异常**。验证 SVG 路径与尺寸、日夜主题、入口间距/无横线、收藏默认/悬停/已收藏状态、拖动上中下位置、相同动画、收藏与取消、刷新/缩窗、键盘、减少动态效果、各自单独安装和图标回退。最终截图已目视检查。首次检查发现通用 hover 仍覆盖内部按钮背景，已修正选择器；最终证据使用 `browser-final/`。
- **共享方案仅评估：** 同步修订 `docs/shared-dock-maintenance.md`：共享组件可放在 NexAlign 仓库的子包里，Aarre 构建时引用；用户安装包各自内置组件，单安装与双安装均可用。推荐正式版本锁定同一组件包，本机可直接引用同一份源码；无需第三仓库，本轮未实施抽包、跨仓库依赖或 CI 修改。
- **接续与交付：** 备份/日志在 `work/dock-brand-refinement-20260914/`，最终浏览器报告在 `browser-final/report.json`，[实际截图对照](work/dock-brand-refinement-20260914/review.html)。NexAlign 版本为 `0.1.0 · 5b81faf3 · 20260914T1436Z`；两边 dist 已更新，既有未提交工作保留，本轮无文件占用。未重载日常 Chrome、修改用户收藏、调用在线 AI 或提交/推送/发布；手机业务未改，本轮未再次运行手机回归。重新加载两只扩展并刷新网页后验收。

# 已完成：浮动把手、纵向拖动与细线图标 0.6.26（2026-09-14，本机）

- **范围与外观：** 按用户要求与 NexAlign 联动修改 PC 入口。把手距可用视口右边 8px，四角 16px；共享为 52 × 100px、独立为 52 × 52px。图标统一 22px / 1.5px 描边、44px 点击区，日夜主题均为中性色，hover / 按下 / 已收藏使用轻量反馈。保留两产品间淡分隔线与 Aarre 悬停收藏；已收藏星标为描边、细边框和小状态点，产品内部品牌资产保持。
- **交互：** 两宿主共用新增 `drag.ts`，右侧仅上下拖动，上下各限位 8px；松手不触发菜单，方向键、Home / End 可调整。位置通过现有 `FLOAT_POSITION` 保存为 `handleRatio`，配对时 Aarre 将位置发布给 NexAlign；连续写入按顺序处理，初始化的旧值不能覆盖新调整。保留主菜单相同高度/动画；展开右侧 8px、四角 20px，独立收藏表单在顶部/底部自动避让。
- **工程验证：** Aarre `package.json` / lock / Manifest / `dist` 均为 **0.6.26**；类型、设计 token、正式构建及 40 个 JS 语法检查通过。5 文件 / **71 项定向测试通过**，涵盖几何、位置协议、客户端、动画、宿主恢复及延迟设置读写；类型补充后宿主恢复 31 项再次通过。NexAlign 类型/UI 边界、正式构建、25 个 JS 等正式产物及包体门通过。
- **真实浏览器：** 独立可见 Chrome 152 中 **45 项通过、0 记录到的运行异常**，覆盖拖动上中下位置、横向限制、两个主菜单开合与实际动画一致、收藏避让/取消/真实写入、日夜主题、22px 图标对齐、刷新、缩窗、键盘、减少动态效果、两只插件各自单独使用。NexAlign 同一构建手机往返 37 项通过；手机业务路径保持。最终截图已目视检查。
- **证据：** 本轮源码备份、日志和早期诊断在 `work/floating-dock-20260914/`；最终双插件报告与截图为 `final-3/`，[实际截图对照](work/floating-dock-20260914/review.html)。无窗口 Chrome 曾在底部 iframe 点击命中出现差异，最终完整验证使用可见窗口；缩窗几何按去除滚动条后的 visualViewport 计算。可复用定向脚本已保存为 `scripts/verify-floating-dock.mjs`，默认独立可见 Chrome。两边八个共享模块逐字节一致。
- **维护方案仅评估：** `docs/shared-dock-maintenance.md` 建议共享源码包加统一构建入口，做到改一份、出两包；本轮没有实施抽包或重构仓库。当前仍需同时更新两个插件，保证配对与单独使用一致。
- **交付边界：** 两边 `dist/` 已更新，本轮无占用文件及待办；未操作日常 Chrome 或用户收藏、未调用在线 AI、未提交/推送/发布。重新加载两只扩展并刷新网页即可验收。旧宽泛验证脚本包含历史外观断言，未将其列为本轮完整通过证据。

# Git 主线交付（2026-09-14，已合并）

- **授权与范围：** 用户要求将当前进展提交、推送并合入主线。`codex/aarre-floating-ui-20260909` 已推送的完整 0.6.25 进展经 [PR #1](https://github.com/ArvenWang/Aarre/pull/1) 于 2026-09-14 13:19 北京时间合入 `main`，合并提交 `5d3733e17bdef79669aa91f04c54346bef265a65`，包含原分支 HEAD `f9885c88db38c623bb7ce20b396340c92a0a0510` 及其全部历史。本机已切回并快进到主线，原功能分支保留，无冲突或强制推送。
- **双项目一致性：** NexAlign 同步经 [PR #2](https://github.com/ArvenWang/LayerScope/pull/2) 合入其 `main`；两边的 7 个共享模块逐字节一致。本次没有再次修改产品行为或增加版本号，Aarre 仍为 0.6.25。
- **本轮验证：** 类型、设计 token、正式构建及全部产物 JS 语法检查通过。构建中的既有 `@theme` 和混合动态/静态导入提示不阻塞产物；本轮未扩大为无关重构。此前各功能的独立 Chrome 和定向测试结果继续保留各自构建边界，没有冒称为本轮全量浏览器复测。
- **完成状态：** 本记录随主线同步。Git 状态备份、PR 正文及远端回执在 `work/git-mainline-20260914/`，`dist/` 已在本机更新；无占用文件及本次待办。未重载日常 Chrome、修改用户收藏、调用在线 AI 或发布商店版本；后续开发从 `main` 的最新状态继续。

# 已交付并推送：缩小品牌字标 0.6.25（2026-09-14）

- **本轮修改：** 图标旁的 Aarre 字标缩小一档：菜单 22→20px（1.25rem），完整收藏库 26→22px（1.375rem）。仅改两处字号 token；保留已选 A / Fraunces Italic、图标、主题配色及明暗白色开关圆钮。
- **验证：** 设计 token 检查、正式构建及产物 JS 语法通过。实际独立 Chrome 15 项目标检查通过，0 页面异常：明暗 420/320px 字体加载、实际字号、图标对齐与无挤压，网页版字号同步，白色圆钮保持。已目视检查新菜单截图。对比页截图、当前字号说明及 shared-dock.md 的现行圆角说明已更新；没有新增复杂测试或跑全量回归。
- **版本与文件：** Aarre 0.6.25，dist 已更新。展示页 docs/design/2026-09-14-brand-type-theme/index.html；本轮备份/构建日志/浏览器报告在 work/wordmark-size-git-20260914/。
- **Git 交付范围：** 用户已授权将当前完整工作区同步远端，包括此前完成的共享 dock、直接收藏/动画、品牌图标与字标、设置抽屉/通用浮层、设计资产、相关测试和进展。当前分支 codex/aarre-floating-ui-20260909，目标为 origin 同名分支。已获取远端状态，origin/main 是本分支祖先，无冲突。被忽略的 dist 和运行临时目录保持本地。
- **Git 已同步：** 完整交付提交 `e6883e6544addb44fdc9995385cfa33cda63db7c` 已推送到 `origin/codex/aarre-floating-ui-20260909`，已通过远端查询核对提交号一致；本次包含 297 个文件，运行临时目录与 dist 未纳入 Git。此条进展随后作为文档提交同步。
- **完成状态：** 本轮完成，无占用文件或待办。未重载日常 Chrome、修改用户收藏或调用在线 AI；未发布商店版本。重新加载扩展并刷新网页后，可使用字号缩小后的 0.6.25。

# 已交付：正式采用 A 字标与白色开关圆钮（2026-09-14）

- **字标：** 用户已确定 A（Fraunces Italic）。新增 AarreWordmark，正式菜单 22px / 完整收藏库 26px，weight 600；仅品牌名使用，正文与会话标题保持原字体。固定 SOFT 100、WONK 1、opsz 24、wght 600，仅保留 Aarre 字形，WOFF 为 2,188 字节并随 CSS 内嵌，无在线字体请求。来源/哈希与轴设置见 selected-font.json，OFL 随扩展分发。
- **开关：** 按追加要求，所有开启/关闭圆钮在日间与夜间均为白色。日间开启轨道保持品牌橙；夜间通过共享 --toggle-active-track 使用中灰，白钮/轨道对比度分别约 3.17:1 / 3.60:1。没有改动主强调色、图标、弹层层次或 NexAlign。
- **验证：** 类型、设计 token、正式构建与产物 JS 语法检查通过。实际独立 Chrome 中 15 项目标检查通过、0 页面异常：明暗 420/320px 正式字体加载与居中、正文隔离、网页版字标、开关两状态均为白钮以及轨道对比度。会话标题已检查渲染分支和 CSS 作用域，未调用 AI 创建会话。未新增测试或跑全量回归。
- **交付：** Aarre 0.6.24，dist 已更新。原对比页 docs/design/2026-09-14-brand-type-theme/index.html 标明 A 已采用，A/设置截图替换为本轮正式扩展截图；保留 B/C/D 回看。字体元数据、OFL、设计说明及截图已保存。源码备份、构建日志和最终浏览器报告位于 work/brand-wordmark-20260914/。
- **边界与下一步：** 本轮完成，无占用文件或待办。未重载日常 Chrome、修改用户收藏、调用在线 AI、提交/推送/发布。重新加载扩展并刷新网页后生效。早期目标脚本修正了 HeroUI 隐藏 input 的操作方式，并移除“新会话会立即进入聊天”的错误验证假设；该入口实际重置为收藏首页，未修改此产品行为。既有未提交工作保留。

# 已交付：品牌字标候选与主题色统一（2026-09-14，字体待选）

- **字标候选：** 4 套真实有机衬线斜体：A Fraunces / B Lora / C Alegreya / D Cormorant Garamond。离线字体、OFL 许可与来源哈希齐备；对比页支持日间/夜间和四方案切换，有放大字形与真实菜单小字号截图。字体只临时应用于独立浏览器截图，正式字标等用户选择；正文和会话标题保持。
- **配色已实现：** 浅色 `--accent` 改深橙 #B74624（白底约 5.36:1），新增控件填色 #F2633D 与现有 logo 一致；暗色强调/填色为中性 #DEDEDE，活跃开关用深色滑块。已收藏、AI 标签、推荐文件夹、同步状态等继承共享 token，独立宿主同步。主操作按钮、错误色、其余图表分类和通用浮层层次保持。NexAlign 与共享 dock 未改。
- **验证：** 设计 token、类型、正式构建及产物 JS 语法检查通过。实际独立 Chrome 中 16 项目标检查通过、0 页面异常，覆盖明暗主题/宿主/开关、四套菜单字体载入与对齐、完整收藏库继承。对比页 4 字体、8 组合与图片载入通过，390/320px 无横向溢出，已目视检查。没有新增单元测试或运行全量回归。
- **交付：** Aarre 0.6.23，dist 已更新。对比页 `docs/design/2026-09-14-brand-type-theme/index.html`，来源/许可/最终实际截图/设计规格同目录；备份、脚本和报告在 `work/brand-type-theme-20260914/`。早期浏览器脚本误把开关 input 当容器，已改为读取实际外层；展示图已修正固有高度，最终截图为成功结果。
- **边界与下一步：** 本轮配色完成，字体待用户选 A/B/C/D 后精简字体包并替换正式字标；无占用中的文件。未重载日常 Chrome、修改用户收藏、调用在线 AI、提交/推送/发布。重新加载扩展并刷新网页后新配色生效；其他未提交工作保留。

# 已交付：设置改为单项展开抽屉（2026-09-14）

- **最新要求已落地：** 六个分类纵向排列，默认 AI 服务展开；展开另一个分类时，当前分类自动收起，也可点击当前项将其折叠。内容直接位于对应标题下方。侧边菜单和完整收藏库共用抽屉结构，取代上一轮的换行页签/左侧分类导航。
- **实现与保留：** 复用现有 HeroUI Accordion 的单项展开、键盘/焦点和开合过渡；内容保持挂载，折叠不丢失未保存输入。移除旧导航/容器查询/切换滚回顶端样式，避免影响新的纵向展开位置。上轮通用浮层颜色、边框、阴影和业务设置流程保留。
- **验证：** 类型、设计 token、正式构建与产物 JS 语法检查通过。复用并更新 settings-groups.integration.test.tsx：1 项通过，覆盖单项展开与未保存 Key 的输入节点/值保留。实际独立 Chrome 中 10 项目标检查通过、0 页面异常，覆盖 420/320px 明暗布局、六分类互斥展开和完整收藏库；已目视检查稳定后的深浅抽屉截图。未跑全量回归。
- **交付：** Aarre 0.6.22，dist 已更新。用户当前展示页 docs/verification/2026-09-13-settings-surfaces/index.html 已替换为最新抽屉截图与说明，原页面备份在 work/settings-accordion-20260914/before-review/。源码备份、构建/测试日志、目标浏览器脚本与记录在 work/settings-accordion-20260914/；测试备份加 .bak，避免被 Vitest 当成用例。
- **边界与下一步：** 本轮完成，无占用文件或待办；未重载日常 Chrome、修改用户收藏、调用在线 AI、提交/推送/发布。重新加载 Aarre 并刷新网页后验收。其他未提交工作保留。

# 已交付：设置分类常驻与通用浮层层次（2026-09-13）

- **需求与结果：** 菜单设置移除六项分类的 Select，下拉分类已不存在。复用常驻导航：侧边常规宽度三列两行、320px 窄宽度两列三行，六项均直接可见；完整收藏库保留左侧分类。隐藏内容保留挂载，未保存的 AI 输入在分类切换后保持。
- **通用浮层：** tokens.css 定义页面/弹窗/弹出菜单表面；暗色分别为 #16181b / #24272a / #34383c，配套统一边框、暗色阴影与提高亮度的弱提示文字。AppModal 默认建立 dialog 表面上下文，内部面板/页脚/控件继承；共享 Select、更多菜单和文件夹选择使用 popover 层级，Portal 自身带层级。浅色保留白色表面，用边框、阴影与现有遮罩区分。完整目的页通过 presentation="page" 保留页面语义。规范在 docs/design/overlay-surfaces.md；不是 SettingsPage 的专用色值补丁。
- **验证：** 类型检查、设计 token 检查、正式构建及产物 JS 语法通过。复用已有 settings-groups.integration.test.tsx（1 项通过），验证未保存输入仍保留。真实独立 Chrome 中最终 15 项目标检查通过、0 页面异常：明暗 420/320px 六分类布局、全部分类切换、内容无横向溢出、弹窗内外背景、模型下拉层级、更多菜单、新建文件夹、完整收藏库左侧分类。已目视检查稳定后的最终明暗截图。新增浮层弱提示文字对比度为弹窗 7.24:1、菜单 5.70:1。
- **过程说明：** 初次浏览器脚本的模型/管理设置按钮名称不匹配，以及独立扩展启动抢先触发造成等待超时，均修正在验证脚本；最终完整运行通过。静态截图已等待浮层入场完成，早期截图不作为最终交付。
- **交付：** Aarre 0.6.21，dist 已更新。真实图集：docs/verification/2026-09-13-settings-surfaces/index.html。源文件备份、构建日志、目标脚本和完整浏览器记录位于 work/settings-visible-tabs-20260913/。此前未提交工作保留，本轮未改 NexAlign 或共享把手模块。
- **边界与下一步：** 本轮完成，无占用文件及待办。未操作日常 Chrome、用户收藏、真实 AI Key 或在线 AI；未提交/推送/发布。重新加载 Aarre 并刷新网页后验收。

# 已交付：内外同心圆角（2026-09-13，按用户最新澄清）

- **最终几何：** 产品图标 36px / **R8**；图标至整条容器边缘 **8px**；外层圆角 **R16 = R8 + 8px**。中间 44px 悬停按钮为 R12，与外框相距 4px。两个应用共享 CSS 参数，外框半径随内层和间距推导，避免后续单独修改造成不均匀。
- **实现：** Aarre 和 NexAlign 的图标裁切统一 R8；Aarre 128u SVG 底板通过显示样式按 `128 × 8 / 36` 精确适配，三个图形路径、颜色及原始 Figma 导出保持。两宿主外框和悬停按钮引用共享参数；展开菜单与动画保持。本轮较早的 R12 外框未交付，已由用户后续同心要求取代。
- **验证：** 两项目正式构建、Aarre JS 语法、NexAlign 25 个 JS 等正式产物检查通过。真实 Chrome **9 项目标检查通过，0 记录到的页面异常**：明暗主题下实际 SVG 底板圆角约 8px（误差小于 0.001px）、左右/上下直边对应 8px，外框与首尾图标可见角的圆心重合；NexAlign 独立宿主一致。实际截图已目视检查，7 个共享文件仍逐字节一致。未新增单元测试或跑无关全量回归。
- **展示修正：** 用户当前的 [展示页](/Users/nefish/Desktop/Coding/Aarre/docs/verification/2026-09-13-brand-icon/index.html) 之前仍是 0.6.17 旧截图，现已更新为最终同心几何及当前真实截图；旧页面完整备份在 Aarre `work/dock-container-20260913/before-review/`，避免旧截图造成误解。
- **交付与边界：** 双方 `dist/` 已更新，Aarre **0.6.20**。两项目 `work/dock-container-20260913/` 保存源码备份和构建日志；Aarre 另有 `verify-concentric.mjs`、最终截图和 `browser/report.json`。既有未提交改动保留，无占用文件及本轮待办；未重载日常 Chrome、改用户收藏、提交/推送/发布。重新加载两只扩展并刷新网页后生效。

# 已交付：侧边把手圆角收紧（2026-09-13）

- 两应用侧边把手外层圆角从 **18px 减为 16px**，Aarre 悬停按钮对应从 14px 调至 12px，维持 4px 同心间距；悬停收藏外壳同步 16px。图标 36px、按钮 44px、展开菜单尺寸与动画保持。
- 最小改动为 Aarre 宿主样式 3 行、NexAlign 宿主样式 2 行；既有目标验证脚本的圆角断言同步。7 个共享模块仍逐字节一致，其他未提交工作保留。
- 两应用正式构建通过；Aarre JS 语法、NexAlign 25 个 JS 等正式产物检查通过。真实 Chrome 中 **7 项目标检查通过、0 记录到的页面异常**，覆盖明暗把手、快捷收藏和 NexAlign 独立宿主；已目视检查实际截图。没有新增单元测试或重跑全量回归。
- Aarre **0.6.19** 与 NexAlign 的 `dist/` 已更新。备份、目标脚本、构建日志与实际截图在各自 `work/dock-radius-20260913/`；Aarre `browser/report.json` 是本轮完整检查记录。
- 本轮完成，无占用文件或待办；未重载日常 Chrome、未改用户收藏、未提交/推送/发布。重新加载两只扩展并刷新网页即可生效。

# 已交付：双应用侧边图标增大、暗色去彩色（2026-09-13）

- **改动：** Aarre 与 NexAlign 的侧边产品图标由 28px 增至 **36px**，保留 44px 按钮、4px 内间距和原把手尺寸/动画。共享与各自独立宿主使用同一尺寸规则，NexAlign 图标裁切圆角随尺寸由 7px 调为 9px。
- **暗色：** Aarre 的深紫/奶油色改为黑色 `#000000` / 中性白 `#F5F5F5`；三个 Figma 路径保持原样。此资产同步作用于把手、收藏菜单和完整收藏库。NexAlign 既有黑白灰保持；浅色图标配色保留。共享模块已同步至两个项目，7 个模块仍逐字节一致。
- **验证：** 两项目类型与各自设计/UI 边界检查通过；Aarre 正式构建及 JS 语法、NexAlign 正式构建/25 个 JS 等产物检查/包体门通过，SourceCanvas gzip 35498 / 36545 B、background 119624 / 120066 B。复用已有真实扩展检查，Chrome 152 中 **17 项通过，0 记录到的页面异常**：双应用明暗图标均 36px、暗色全部中性 RGB、悬停收藏、菜单/收藏库深色资产、刷新主题，以及两只扩展各自单独使用。最终把手截图已目视确认。未新增单元测试或重跑无关全量回归。
- **交付：** 双方 `dist/` 已更新，Aarre **0.6.18** / NexAlign **0.1.0 · 2a7bd4ac · 20260913T1325Z**。[实际对照](/Users/nefish/Desktop/Coding/Aarre/docs/verification/2026-09-13-icon-mono-size/index.html)。两项目备份和工程日志在 `work/icon-mono-size-20260913/`，Aarre 下另含浏览器脚本、截图和颜色/几何一致性记录。保留原 Figma 导出及此前验证页面作为历史证据，生产暗色为用户本轮要求的中性适配版。
- **完成边界：** 无占用文件及本轮待办；其他未提交改动保留。未操作日常 Chrome、未写入用户收藏、未调用在线 AI、未提交/推送/发布；重新加载 Aarre 与 NexAlign 并刷新网页后生效。

# 已交付：拾集 A1 正式产品图标（2026-09-13）

- **用户授权与范围：** 将用户确定的 Figma「拾集 · A1 聚拢」导出用于真实 Aarre 产品，并匹配深浅模式。Aarre 更新扩展 PNG / SVG、完整收藏库、悬浮菜单和共享把手；NexAlign 同步共享图标模块。没有修改既有菜单、收藏操作和动效。
- **正式资产：** 浅色为 Poppy 暖橙 `#F2633D`，深色为 Aubergine `#392B3A`，图形均为 Butter `#F9F0D5`。工具栏与标签页使用暖橙通用版；16px PNG 为 Figma Micro，32 / 48 / 128px 为标准版。两份 SVG 与四份 PNG 在 public / dist 中均与 Figma 原始导出逐字节一致；共享宿主嵌入相同 SVG，避免依赖网页图片权限。
- **界面接入：** `AarreIcon` 复用现有 `data-theme`，菜单 24px、收藏库 30px、把手 28px；移除旧品牌书签轮廓及其黑底/缩放样式。功能星标、文件夹图标和 NexAlign 图案保留；7 个共享模块继续逐字节一致。
- **验证：** Aarre 类型、设计检查、正式构建及 JS 语法通过；已有 3 文件 / 14 项定向测试通过。NexAlign 类型/UI 边界、正式构建、25 个 JS 等产物检查与包体门通过，SourceCanvas gzip 35497 / 36545 B，background 119624 / 120066 B。早期以开发构建运行正式包检查的失败是构建模式不匹配，最终已使用 `build:release` 重新验证，不作为未解决产品问题。
- **实际画面：** 独立 Chrome 152 中最终 13 项通过、0 记录到的页面异常：明暗配色、正确 SVG 载入、单个可见图标、实际尺寸、主题同步/刷新、悬停收藏入口和单独安装 Aarre。最终截图已目视检查；初次截图包含过渡/加载状态，仅 `final-browser/` 为最终证据。
- **交付与证据：** 双方 `dist/` 已更新，Aarre **0.6.17**；NexAlign **0.1.0 · 2a7bd4ac · 20260913T1316Z**。[实际截图对照](/Users/nefish/Desktop/Coding/Aarre/docs/verification/2026-09-13-brand-icon/index.html)；导出来源 `docs/design/2026-09-13-aarre-icon-master/`；两仓库备份与工程日志在各自 `work/brand-icon-20260913/`，Aarre 内含定向浏览器脚本及 `assets-verified.json`。
- **完成边界：** 本地实现与上述验证已完成，无占用中的文件及本轮待办。未重载日常 Chrome、未调用在线 AI、未写入用户收藏、未提交/推送/发布；需重新加载 Aarre 与 NexAlign 并刷新网页。没有重跑不相关的全量回归。

# 拾集 A1 聚拢：Figma 矢量精修已交付（2026-09-13）

- **本轮范围：** 用户已确定「拾集 · A1 聚拢」，要求不用生图 API，在 Figma 中用可编辑矢量与尺规构造细化 icon。仅制作正式设计稿与导出资产，尚未要求替换插件。
- **已准备：** 已查看 A1 原图；自主绘制三块不规则圆润石形，以 9 个圆和公切线构造轮廓，调整三处间隙；完成标准与 16–20px Micro 几何版本及本地目视草稿。`docs/design/2026-09-13-aarre-icon-master/` 保存坐标、构造算法、预览及待执行的 Figma 原生矢量脚本。
- **用户指定位置：** 继续在 `u5KlPWzuoMDQBPdO0lP6Mp` / DesignThinking 的 `minitools` 页面（`5203:2821`）制作，参考 Align 节点 `7364:8824`。已读取其原图及附近节点，确认原稿下方 `x=95920, y=640, w=1480, h=2860` 没有已有节点，直接新增 Aarre 工作区，无需新建文件或再选择空间。
- **Figma 交付：** [成稿](https://www.figma.com/design/u5KlPWzuoMDQBPdO0lP6Mp/DesignThinking?node-id=7387-8839) 为 1440 × 1546，位于 x=95920 / y=640；[组件母版](https://www.figma.com/design/u5KlPWzuoMDQBPdO0lP6Mp/DesignThinking?node-id=7390-8878) 为 1440 × 534，位于其下方 y=2266。含三个独立矢量形状、九个原生构造圆、公切线和半径标记；2 个标志母版、6 个图标变体，展示使用真实实例，绑定四个 Aarre 品牌颜色变量。
- **验证与导出：** 已目视查看最终 Figma 展示稿、母版和真实尺寸样例。16 / 20px 使用 Micro；24 / 32 / 48 / 64 / 128px 使用 Regular。导出 8 个 SVG（均三个路径）和 7 个 PNG（尺寸均符合文件名），`exports.json` 含哈希。新增 Figma 内容无图片填充；未使用生图 API。`figma-state.json`、`figma-verification.json` 与最终截图支持继续编辑和复查。
- **完成边界：** Align 原节点 `7364:8824` 位置、尺寸和六个子节点保持不变，既有图稿没有被覆盖。仅设计稿与导出素材；插件代码、图标、构建与安装态未替换，无占用中的实施文件，无本轮剩余待办。

# 三个入选方向的六套品牌延展已交付（2026-09-13，待用户比较）

- **用户选择：** 拾集、回流、林间三个方向可以继续发展，要求基于它们延展供查看；本轮仍为视觉方案选择阶段。
- **执行：** 已用三个原始品牌板作视觉参考，通过内置 `image_gen.imagegen` 各生成两个完整延展：拾集 A1 聚拢 / A2 拼合，回流 B1 方环 / B2 旋流，林间 C1 新芽 / C2 林隙。保留各自配色和核心图形语言，探索轮廓、字标、图形、深浅底及产品应用。
- **交付：** [三个方向、六套延展对照页](docs/design/2026-09-13-aarre-brand-refinements/index.html)。每组附原方向缩略图及两个新方案，可打开完整图；六张新 PNG 均为 1536 × 1024，副本与生成原文件逐字节一致。目录含 `prompts.json`、`outputs.json`（参考/生成来源、尺寸、哈希）、`comparison.png` 和 `mobile.png`，三个参考图已复制到同目录。
- **确认：** 已目视查看原参考、新图及桌面/手机对照页；1280px 和 390px 下无横向溢出，3 个分组、6 个新方案、9 处图片和页内链接正常。图中应用及尺寸样例为概念表达，不等于正式 UI 或已完成真实小尺寸图标验收；原十套、生产代码、插件资产与构建均未改动。
- **下一步：** 等用户继续比较或选定具体延展，再整理正式标志、字标与应用资产；无占用中的代码文件。

# 十套品牌视觉已交付（2026-09-13，待用户选型）

- **用户要求：** 在已有四套基础上继续探索，总共需要十套品牌视觉。
- **执行：** 本轮通过内置 `image_gen.imagegen` 六次独立生成，补齐 05 林间、06 索引、07 余波、08 剪藏、09 藏印、10 闪存；每套包含标志、字标、配色、字体、图形与产品应用示意。既有 01–04 原图完整保留。
- **交付：** [十套完整对比页](docs/design/2026-09-13-aarre-brand-directions/index.html)，05–10 标记新增，可直接跳转大图。六张新增原图均为 1536 × 1024；`prompts.json` / `outputs.json` 已包含十套记录，本轮另存 `prompts-05-10.json` / `outputs-05-10.json`；`comparison-all.png` 为十套总览截图。
- **确认：** 已逐张目视检查六张新图，并查看十套总览及手机浏览截图。十张 PNG 的哈希全部与记录一致，新增副本与工具原始文件逐字节相同；1280px / 390px 下无横向溢出，十套卡片、十个详情、二十处图片与页内链接正常。仅设计候选与浏览页面，未修改插件代码、资产、构建或安装态，未运行无关回归。
- **下一步：** 等用户比较并选择品牌方向，再整理正式资产；无占用中的代码文件。

# 四套完整品牌视觉已交付（2026-09-13，待用户选型）

- **用户要求：** 改为完整品牌视觉、多套独立风格，自主定调，不依赖之前图标方向。
- **范围与执行：** 四次独立调用内置 `image_gen.imagegen`，已生成藏阅（温暖编辑）、回流（鲜明数字）、拾集（轻松收藏）、珍本（艺术品牌）四套身份；每套包含标志、字标、字体、配色、图形与产品应用示意。
- **交付位置：** [完整品牌视觉对比页](docs/design/2026-09-13-aarre-brand-directions/index.html)。四张 1536 × 1024 PNG 保留工具原始输出，副本与原文件逐字节一致；同目录 `prompts.json`、`outputs.json` 记录提示词、来源、尺寸和 SHA-256，`comparison.png` 为对比页截图。
- **确认：** 已目视检查全部品牌图及对比页；1280px 和 390px 下图片加载正常、页面没有横向溢出。图中界面与字体均为概念示意，未声称已实现或形成可授权使用的完整字体文件；本轮未改生产代码、构建或安装态，未运行无关回归。
- **下一步：** 等用户选择整体气质，再按选定方向整理正式字标、图标和视觉资产；无占用中的代码文件。

# 重新生成已交付：完整宝石轮廓（2026-09-13，待用户反馈）

- **用户要求：** 上一轮方向完全不接受，要求重新生成。本轮再次直接调用 `image_gen.imagegen`，转向完整的宝石轮廓、中央主切面及少量边缘切面。
- **生成与取舍：** 四次生图输出；01 蓝色过于像方块、03 紫色偏向标签，已自行淘汰为推荐项。保留 **02 斜切琥珀** 与 **04 红色原石** 供用户判断新方向，分别采用同一琥珀/红色色系的少量切面。已目视查看所有原始输出，尚无用户选定方案。
- **交付：** [新方向原图](docs/design/2026-09-13-aarre-gem-restart/README.md)。同目录四张原始 PNG 完整保留，`prompts.json` / `outputs.json` 记录生图提示词、保留/淘汰状态、原始文件路径与哈希；没有用代码重绘或改变图片。
- **边界与下一步：** 等用户反馈方向，再精修轮廓、透明背景与小尺寸适配；本轮候选不声称已是可直接集成的最终图标。插件资产、代码、构建与日常安装态均未修改，未运行无关回归，无占用中的代码文件。

# 生图工具原始候选已交付（2026-09-13，待用户选择）

- **调用与范围：** 按用户明确要求，直接调用内置 `image_gen.imagegen` 生图工具接口，共四次独立生成，并通过工具原生图片输出展示给用户。此处是内置工具接口，没有配置或声称调用单独的 OpenAI REST API / CLI。
- **输出：** 01 原矿、02 斜晶、03 润玉、04 藏晶；青绿色单色、白底、少量切面、不规则宝石。四张原图已目视检查，白底与色块正常。工作区 PNG 与工具原始文件逐字节一致，没有重绘、调色、裁剪或另做对比网页。
- **交付文件：** [原图与生成描述](docs/design/2026-09-13-aarre-gem-direct/README.md)，同目录 `01.png` 至 `04.png`、`prompts.json` 与 `outputs.json` 记录原始提示词、生成文件路径及 SHA-256。此前候选保留。
- **下一步与边界：** 等用户选型或继续反馈；当前仅为设计候选。插件代码、图标、构建和已安装扩展均未修改，未运行无关回归，无占用中的代码文件。

# 第二轮图标候选已交付：单色不规则宝石（2026-09-13，待用户选型）

- **方向与边界：** 用户未选用首轮方案，要求收敛成代表“宝藏”的不规则宝石，优先 mono 单色。本轮统一青绿色，用留白区分少量切面；只交付设计候选，不替换插件或改动构建。
- **四个方案：** A 原石（紧凑、棱角与偏心切面）、B 斜切（一道主要切面）、C 润石（柔和轮廓与几何切面）、D 晶块（横向不规则晶体）。通过内置 `image_gen` 生成；A/B/C 初稿的局部透光斑与明暗已用同工具精修，原稿单独保留，不作为最终预览。
- **文件与交付：** [单色宝石候选对比](docs/design/2026-09-13-aarre-gem-icons/index.html)。最终使用 `gem-a-raw-flat.png`、`gem-b-oblique-flat.png`、`gem-c-soft-flat.png`、`gem-d-shard.png`，总览 `comparison.png`；设计描述和精修提示词在 `prompts.json` / `refinement-prompt.txt`。Codex 原生成文件未删除，工作区副本可独立查看。
- **视觉确认：** 已目视检查最终大图、16/24/28/48px 预览与 28px 把手比例示意。浏览器内所有图片和链接可用，1280px 和 390px 页面无横向溢出，把手示意宽 52px。比例示意仅用于选型，不代表插件已替换；最终正式资产仍需用户选型后精修为纯色矢量、去除白底并适配小尺寸。
- **下一步：** 等用户选择或继续反馈轮廓/色彩；A 更硬朗，C 更圆润。当前插件代码、已安装扩展及 `dist/` 保持原样，没有进行无关回归或发布，无占用中的代码文件。

# 首轮图标候选：用户未选用（2026-09-13）

- **范围：** 参考 NexAlign 的正式灰阶图标，使用内置 `image_gen` 绘制 Aarre 新图标候选；按用户“多给几个版本，我来选”的要求，本轮止于候选交付。
- **候选：** A 叠页书签、B 折角卡片、C 收藏匣、D 折带 A。四张原始 PNG、NexAlign 对照、完整提示词及并排对比页保存在 `docs/design/2026-09-13-aarre-icon-concepts/`。原生成文件保留在 Codex generated_images 目录，工作区副本独立可用。
- **视觉检查：** 已目视检查四版大图与把手比例示意；对比页展示 16/24/28/48px。候选含透明区域，统一用黑色图标底板展示，未改原始图片。Chrome 中所有图片正常加载，PC 1280px 与手机 390px 页面无横向溢出，比例示意把手宽 52px。该示意是设计比较，不是已替换后的扩展运行截图。
- **交付：** [四版候选对比](docs/design/2026-09-13-aarre-icon-concepts/index.html)，`comparison.png` 为对比总览。推荐先比较 A 的收藏识别与 D 的品牌符号方向。
- **下一步：** 等用户选定编号后，精修正式矢量轮廓、适配小尺寸与明暗主题，再替换两端共享把手和扩展资源。当前插件代码、图标与构建均未修改，未运行与设计候选无关的回归，无占用中的代码文件。

# 当前交付：产品图标把手与悬停收藏入口（2026-09-13，已完成）

- **产品行为：** 默认仅显示已安装产品的正式图标，双安装为 Aarre 与 NexAlign 两个入口。原 SVG 图形和品牌填色直接内嵌，支持 NexAlign 明暗版本，没有新增资源权限或跨仓库构建依赖。悬停 Aarre 时向左展开收藏星标；直接点击产品仍打开对应菜单。已收藏保持实心星标，真实添加、管理和取消沿用已有流程。
- **尺寸与交互：** 外层圆角从 20px 微调为 **18px**，按钮为 **44×44 / 14px 圆角**，四周 4px 间距。双产品把手 52×100，单产品 52×52。鼠标跨越间隙仍可点击收藏；Tab 聚焦、左方向键进入星标、Esc 收起快捷操作均可用。鼠标收起主菜单后的焦点恢复不再意外展开快捷操作。
- **实现边界：** 修改 Aarre 宿主及样式、共享图标与高度，NexAlign 宿主使用同一产品图标。7 个共享模块逐字节一致，`geometry.ts` / `morph.ts` 未改，保留主菜单与收藏表单的独立尺寸、280ms 展开 / 210ms 收起与现有动画优先路径。原有未提交升级均保留。
- **实际验收：** 同一 Aarre **0.6.16** / NexAlign **0.1.0 · 2a7bd4ac · 20260913T0536Z** 在独立 Chrome **152.0.7977.84** 中 **22 项通过、0 记录到的页面异常**。覆盖两个真实图标、悬停及间隙、键盘、实际写入一条临时收藏、取消不写入、重复进入不重复保存、菜单切换/动画、深浅主题、360px 窄窗及双方单独安装。截图已目视检查，单产品截图等待已卸载的旧宿主清理完成后再拍摄。
- **工程验证：** Aarre 定向 **59 项 / 3 文件**、类型与设计检查、正式构建及全部 JS 语法通过；NexAlign 类型/UI 边界、正式构建、25 个 JS/Manifest/许可证/SBOM 检查与包体门通过。SourceCanvas gzip **35487 / 36545 B**，background **119624 / 120066 B**，未提高额度。没有重跑无关全量回归。
- **交付与接续：** 双方 `dist/` 已更新；[实际截图与录屏](docs/verification/2026-09-13-product-handle/index.html)。最终浏览器证据为 Aarre `work/product-handle-20260913/final-visible/`，两仓库备份与工程日志在各自 `work/product-handle-20260913/`。早期目录仅用于诊断，旧验证器中常驻星标/三行把手断言已适配；完整旧双插件脚本本轮仅语法检查，未作为本轮运行证据。
- **完成边界：** 本地实现、构建与上述验证已完成，无占用中的实施文件。需用户重新加载 **Aarre 与 NexAlign 两只扩展** 并刷新网页；未操作日常 Chrome 的收藏，未调用在线 AI 服务，未验收物理手机或全部业务回归，没有提交、推送或发布。热卸载到宿主清理之间的既有过渡时间没有在本轮改动。

# 当前交付：主收藏菜单点击即展开（2026-09-13，0.6.15 已完成）

- **原因与修复：** 主入口仍等待协同 OPEN 回执后才显示外框，未复用星标提前展开的路径。本轮把提前展示移入共用打开函数：主菜单与星标均在点击时开始动画，内容/连接继续加载。主菜单读取不再阻塞协同打开回执，避免加载中关闭后协同状态滞后；等待协调时关闭会拒绝迟到的打开命令。重新打开也会取消旧收起定时器。
- **复现与实际结果：** 独立 Chrome 152 / Aarre **0.6.15** / NexAlign **1751Z** 中，刻意延后真实协调消息 1 秒、Chrome 读取 2.3 秒。相同条件下，0.6.14 首次约 **1066ms** 才启动动画；0.6.15 约 **56ms** 启动、**63ms** 出现首个尺寸变化，内容约 **3.56 秒** 后就绪。再次/取消后打开约 **13–15ms** 启动。数据和身份校验没有伪造，旧基线还复现了加载中关闭后重开无响应。
- **验证：** 目标宿主 **27 项测试通过**（修复前新增主入口用例失败），类型检查、正式构建及 JS 产物语法通过；真实浏览器 **15 项检查通过，0 记录到的页面异常**，覆盖首次/重复、内部加载、加载中关闭、迟到回执、再打开、星标保存页面与取消。加载与完成截图已目视检查，实际录屏已保存。7 个共享模块仍与 NexAlign 逐字节一致，保留 280ms 展开 / 210ms 收起；未重跑全量回归。
- **交付：** `dist/` 已更新为 **0.6.15**；[实际录屏与时序](docs/verification/2026-09-13-main-animation-first/index.html)。源码备份、旧构建复现与最终日志在 `work/main-first-motion-20260913/`；`verified/` 为最终浏览器证据，`baseline/` 为旧构建失败记录。复现命令：`SUITE_VERIFY_MAIN_FIRST=1 SUITE_ARTIFACT_DIR=work/main-first-motion-20260913/new-run node scripts/verify-floating-save-transition.mjs`。
- **边界：** 没有修改另一项目或覆盖其他未提交升级；没有重载日常 Chrome、调用在线 AI、写入用户收藏、提交/推送/发布。重新加载 Aarre 后即可体验。无待办、无占用中的文件；独立测试浏览器与服务已关闭。

# 此前交付：动画先行，收藏内容随后加载（2026-09-13，0.6.14 已完成）

- **原因与修复：** 0.6.13 等待页面读取/表单提交后才展开；双插件还先等待另一插件的关闭回执。本轮点击星标即开始收藏外壳动画，协同确认与数据准备在后台继续；确认后才接入 iframe，未改动双方身份校验或共享模块。
- **最终行为：** 收藏使用 360 × 560px 的紧凑外框（短窗自动收进视口），先显示可关闭、可取消的加载状态，再在原尺寸内显示真实表单；长内容在内部滚动，不再因加载完成或 AI 结果二次放大。仍沿用主菜单的 280ms 展开、210ms 收起与同一缓动。加载中关闭后，晚到回执/读取不会重开；新的打开指令可覆盖先前取消。
- **取消边界：** 加载取消后重新建立 iframe 身份，避免旧初始化回复复用已销毁窗口的连接；替换帧在通过身份校验后，仅清除该已取消的保存草稿，保留其他编辑与对话。修复中保留了既有未提交改动，未改动另一项目。
- **最终验证：** 独立 Chrome 152 / Aarre **0.6.14** / NexAlign **1751Z** 的 **60 项检查通过，0 记录到的页面异常**。真实 Chrome 读取刻意每次延后 2.3 秒，冷启动内容共等待约 **9.46 秒**，动画仍在约 **60ms** 启动、**82ms** 出现尺寸变化；其他收藏场景约 **3–19ms** 启动。首次、重复、切换后、窄窗、单插件均保持共享动画与稳定外框；加载中取消、重开主菜单、关闭后切换正常，取消没有写入书签。加载与完成截图已目视检查。
- **代码检查：** 宿主最终 **25 项**与其他 4 文件 **43 项**针对性测试通过；类型、设计检查、正式构建及产物语法检查通过。没有重跑全量回归。此前诊断包括旧身份重开失败、测试配置的首次引导以及协调等待时序；仅 `work/save-first-motion-20260913/delivered/` 代表最终浏览器结果。
- **交付与边界：** `dist/` 已更新为 **0.6.14**；[实际录屏与时序](docs/verification/2026-09-13-animation-first/index.html)。日志和备份在 `work/save-first-motion-20260913/`。未重载日常 Chrome、未调用在线 AI、未重测云同步/手机、未提交/推送/发布；重新加载 Aarre 后即可体验。无待办、无占用中的文件；独立浏览器与服务已关闭。

# 此前交付：恢复统一收藏开合动画（2026-09-13，0.6.13）

- **修正上轮理解：** 一步展开到收藏页应保留主菜单相同的动画；0.6.12 直接取消动画不符合用户要求。本轮恢复直接调用共享动画控制器，七个共享模块与 NexAlign 逐字节一致，另一项目没有修改。
- **实际行为：** 内容与真实尺寸准备好后才开始一次连续展开。收藏页与主菜单底板展开 280ms、收起 210ms、同一缓动；内容延后 100ms 平移淡入 140ms、退出平移淡出 120ms。内层弹层保持不透明、不额外缩放，不透出首页。取消/关闭保留表单直到宿主确认收起完成；快速反向打开取消旧清理请求，防止晚到回执清掉草稿。
- **验证：** 6 文件 / **44 项针对性回归**、类型、设计检查、构建与产物语法检查通过。独立 Chrome 152 加载 Aarre **0.6.13** 和 NexAlign **1751Z**，**51 项浏览器检查通过、0 记录到的页面异常**：首次、重复、切换后、窄短窗口及单插件的动画时长/延迟/缓动均与实际主菜单一致，存在连续中间帧，表单第一可见帧尺寸正确；退出后清理，取消未写入书签。收起中间帧和最终画面已目视检查。
- **交付：** `dist/` 已更新为 **0.6.13**；[录屏与对照](docs/verification/2026-09-13-save-motion/index.html)。本轮日志与原始录屏在 `work/save-motion-20260913/`，修改前备份在 `before/`。既有未提交升级工作保留。
- **边界与下一步：** 未重跑全量回归、在线 AI、云同步或手机；未重载日常 Chrome，未提交/推送/发布。用户重新加载 Aarre 即可复核动画。无实施待办、无占用中的文件，测试 Chrome 与服务已关闭。

# 此前交付：收藏表单直接展开与关闭（2026-09-13，0.6.12；动画已由 0.6.13 修正）

- **已复现并修复：** 原 0.6.11 先把入口展开成 448px 占位窗口，再调整为实际表单（本次 545px）；逐帧截图另捕捉到弹层外层淡入让下方首页透出。不是只更改动画时长。
- **最终行为：** 当前请求的真实表单及内容尺寸就绪后，一次显示最终尺寸、完全不透明的收藏页；收藏打开不再拉伸或淡入，取消/关闭立即回到右侧按钮。准备中取消不会被晚到消息重新打开，超时回到可操作入口。在线 AI 仍可异步完成，不作为打开条件。
- **保留范围：** 两插件共享入口、主菜单动画、收藏草稿和原有业务流程保留。`src/shared/suite-dock/` 七个模块与 NexAlign 逐字节一致，没有修改另一项目。既有未提交升级改动保留，修改前备份位于 `work/save-transition-20260913/before-source/`（.bak 防止被测试发现器误扫）。
- **最终验证：** 类型、设计检查、正式构建及 JS 语法通过；6 文件 / **43 项针对性回归**通过。独立 Chrome 152 加载 Aarre 0.6.12 与 NexAlign 1751Z，**25 项真实浏览器检查通过、0 记录到的页面异常**：首次、重复、切换后、窄短窗和单插件均无小窗口拉伸帧，表单及外层无透明/缩放，关闭后的第一帧已回到手柄；取消没有写入书签。首帧、窄窗及收起截图已目视确认。
- **交付：** 本项目 `dist/` 已更新为 **0.6.12**；[录屏、截图与记录](docs/verification/2026-09-13-direct-save/index.html)。可复现脚本 `scripts/verify-floating-save-transition.mjs`。最终原始证据在 `work/save-transition-20260913/verified-final/`，其他运行目录是排查中间态。
- **边界与下一步：** 本轮不重跑全量回归、联网 AI、手机或云同步；未重载日常 Chrome、未提交/推送/发布、未覆盖旧 outputs 封装。重新加载 Aarre 扩展即可复核。无实施待办、无占用中的文件；独立测试浏览器和服务已关闭。

# 当前交付：移除切换栏，统一主菜单高度和动画（2026-09-13，已完成）

- **用户要求已完成：** 删除两个菜单顶部的应用切换 tab 及其 36px 占位，NexAlign 关闭按钮回到本身页头。共享手柄的三个按钮及淡分隔线保留；切换时收起当前菜单，再点另一入口。主菜单高度统一，收藏表单按内容独立计算。
- **原因与实现：** 此前底板参数相同，但 NexAlign 内容仅淡入/淡出，退出 100ms；Aarre 在宿主内另写平移动画，退出 120ms，就绪/状态更新的取消方式也不同。现在两边直接调用逐字节一致的 `src/shared/suite-dock/geometry.ts` 和 `morph.ts`，删除宿主内重复动画。主菜单为可用视口高度减去上下各 12px；底板展开 280ms、收起 210ms，同一缓动；内容平移并淡入 140ms（延后 100ms）、退出平移淡出 120ms。反向操作从当前形状和透明度继续，重复消息不重启动画，过渡中尺寸变化保持原截止时间；减少动态效果时立即切换。
- **构建：** NexAlign `0.1.0 · 2a7bd4ac · 20260912T1751Z`、Aarre `0.6.11`，两项目各自 `dist/` 已更新。仅调整 PC，手机原有路径保留；没有提交、推送或发布。原有未提交工作保留，备份 `work/dock-parity-20260913-014602/before/`。
- **双插件实测：** 独立 Chrome `152.0.7977.84` 中 **43 项全部通过，0 记录到的运行异常**。两边实际开合的底板与内容关键帧、时长、延迟、缓动完全相同，均有连续尺寸和透明度中间帧。900px 窗口下两主菜单均为 876px，480px 短窗口下均为 456px；收藏表单本次为 545px。无顶部切换栏、三个入口、实际收藏/取消、已有收藏反馈、主题、草稿、关闭后切换、调试连接、持续加载页和卸载回退均通过。
- **其他验证：** Aarre 类型、设计检查及 6 文件 / 67 项通过，覆盖动画反向、重入、过渡重定向和减少动态效果。同一 NexAlign 构建的暂停清理 30 项、手机往返 37 项通过，0 记录到的运行异常。两边构建和类型/UI 检查通过；NexAlign 25 个 JS 等正式产物与包体门通过，SourceCanvas gzip 35003 / 36545 B、background 119624 / 120066 B，未提高额度；7 个共享模块逐字节一致。没有重跑此前全量回归。
- **证据：** LayerScope 的 `work/dock-parity-20260913-014602/` 中有 `dual/report.json`、`pause/report-fixture.json`、`mobile/report-fixture.json`、`verification.json` 和 `review.html`。主菜单与短窗口截图已目视检查。动画比较从同一静止手柄开始；提前点击引发的正常反向形变诊断另存。关闭后切换按实际 attach/detach 完成事件检查，幂等关闭消息和跨扩展权限拦截不代表仍然连接。
- **交付边界：** 本地实现、构建与验证完成，无占用中的实施文件。日常 Chrome 尚未由代理重载，需重新加载两只扩展并刷新网页。既有跨机器分发身份限制仍见 `docs/shared-dock.md`。

# 此前交付：共享手柄分组与收藏入口修复（2026-09-13，已完成）

- **问题与改动：** 旧共享实现用 NexAlign 替换了 Aarre 的收藏星标。现在依次保留 Aarre 展开菜单、收藏星标、淡分隔线和独立 NexAlign 按钮；展开后的应用切换栏也有弱分隔线。星标沿用真实收藏/取消/已有收藏管理流程，并恢复已收藏时的实心反馈。
- **外观与范围：** 共享入口为 52 × 153px，三个按钮均为 44 × 44px，分隔线为 24 × 1px、随统一明暗主题调整。两个宿主的展开与收起尺寸同步；单独 Aarre 保留原来的两个按钮和 52 × 100px。仅修改 PC 共享路径，手机原有路径未改。
- **构建：** 本项目 `dist/`，Aarre `0.6.10`；本次联调组合为 NexAlign `1727Z` + Aarre `0.6.10`。没有提交、推送或发布；已有未提交工作完整保留，修改前备份为 `work/shared-dock-actions-20260913-012130/before/`。
- **针对性验证：** Aarre 类型、设计边界及 6 文件 / 72 项通过；NexAlign 类型/UI 边界、25 个 JS 等正式产物检查、包体门通过（SourceCanvas gzip 35301 / 36545 B，background 119624 / 120066 B，未提高额度）。两边构建与 diff 检查通过，共享模块逐字节一致。没有重跑或宣称此前 601 项全量验证覆盖新构建。
- **真实双插件：** Chrome `152.0.7977.84` 独立配置中 **33 项全部通过，0 记录到的运行异常**。三个入口、取消无写入、真实 Chrome 书签添加、实心星标、管理已有收藏不重复添加、双向主题、草稿恢复、连续动画、持续加载页以及双方卸载回退均通过。收藏用例在临时 Chrome 内建立实际文件夹并真实写入，未改动日常书签。报告 兄弟项目 `../LayerScope/work/shared-dock-actions-20260913-012130/dual/report.json`；图集在 LayerScope 的 `work/shared-dock-actions-20260913-012130/review.html`，明暗手柄和收藏表单已目视检查。
- **验证维护：** 新增装饰分隔线后，旧缩放用例改为精确定位可操作的宽度分隔条；收藏用例限定真实表单操作区以避开无障碍关闭按钮，深色截图等待收起动画完成。早期定位/测试配置诊断单独保留，不作为最终通过证据。
- **交付边界与下一步：** 两份本地构建已更新，日常 Chrome 尚未由代理重载；需重新加载两只扩展并刷新网页。旧版独立分发身份限制沿用 `docs/shared-dock.md`。无占用中的实施文件，本次两个修复无待办。

# 此前交付：共享 PC 手柄与连续展开（2026-09-13，已完成）

- **产物与范围：** Aarre 已升级并正式构建为 `0.6.9`，位于本项目 `dist/`；与 NexAlign `0.1.0 · 2a7bd4ac · 20260912T1706Z` 协同。用户仅授权 PC，手机交互保持原样。此前收藏表单标题对齐及其他未提交工作保留；本轮前备份 `work/shared-dock-20260912-234609/before/`。未提交、推送、部署或发布，既有 outputs 封装仍为历史版本。
- **实际功能：** 两只插件在同一页面允许显示入口时共用一个右侧手柄，提供两个可见的应用图标；打开后的同样一排入口用于切换工作区。中性底板连续展开/收起，协调回执不再截断动画。只剩一只插件或连接失效时恢复独立入口。共享代码分别内置在两个仓库 `src/shared/suite-dock/`，可各自构建。
- **配色：** 入口及切换栏统一中性色，明暗采用最后一次手动设置，启动和旧值恢复不算新选择；旧云端主题不能覆盖共享新记录。NexAlign 的跟随系统在 Aarre 同样解析。旧设置无历史时间，第一次连接确定性迁移；手动再选一次后即按最新选择统一。网页本身的配色仍单独控制。
- **交接可靠性：** Chrome 会禁止另一扩展调试仍包含 Aarre iframe 的页面，即使 iframe 已隐藏。因此先完成本地草稿/收藏表单/列表状态保存，再释放 iframe；回来时保留当前视图并申请新帧身份。AI 设置未保存、操作进行中或保存失败会保留原面板并说明原因。NexAlign 先清除覆盖层、暂停并释放 debugger 才交接。网页仍在加载时立即注入入口；清理容忍失效连接，并主动探测未被 Chrome 通知的卸载。
- **信任边界：** Aarre manifest 只允许明确的两个 NexAlign ID，校验浏览器提供的顶层文档身份；没有网页 matches、新增权限或业务数据跨扩展传输。当前本机双方 ID 已实测；其他机器不同路径的未打包 NexAlign 可能不同，统一分发前需固定其既有商店身份。详见 `docs/shared-dock.md`。
- **自动验证：** 全量 **109 文件 / 601 项通过**，记录 `work/shared-dock-20260912-234609/tests-release.log`；随后新增无卸载通知的探活用例，最终相关 7 文件 / 37 项通过，记录 `targeted-final.log`。类型、设计 token、正式构建与 JavaScript 产物语法检查通过，最终源码 diff 检查通过；未把这次新增用例算成已经重跑的 602 项全量。
- **真实双插件验收：** 最终两个 dist 在独立 Chrome `152.0.7977.84` 中 **24 项通过，0 记录到的扩展运行异常**，覆盖图标可见、主题最后设置、双向切换、草稿恢复、NexAlign 调试重连、手动收起后切换、两边分别卸载、重新配对及持续加载页面。逐帧确认 Aarre 收起有 10 个中间尺寸帧，最终回到 52 × 100px 入口。报告与目视检查后的图集位于兄弟仓库 `../LayerScope/work/shared-dock-20260912-234609/{dual/report.json,review.html}`。
- **交付边界：** 本地实现、构建与本轮验证完成，无占用中的实施文件。日常 Chrome 尚未重载本轮两只插件；重新加载各自 dist 并刷新网页后体验。当前云端配置沿用生产构建配置，真实云同步/Provider 调用、其他系统、手机及商店发布未重新验收。

# Aarre 项目进展

最后更新：2026-09-12（收藏表单标题对齐小修已完成）

## 此前小修 · 收藏表单标题对齐

- **已完成：** 收藏表单标题栏改为垂直居中，修复标题高于关闭按钮的问题；只修改 `src/ui/floating/floating.css` 一处布局规则。
- **验证：** 浏览器实际画面已目视确认，标题和关闭按钮中心均为标题栏内 y=24px；本地构建通过，已更新 `dist`。按用户要求未新增测试、未运行全量回归。
- **交付边界：** 此项为 0.6.8 本地样式修订，重新加载 Chrome 扩展后生效；本轮未操作已安装扩展。下列封装产物及原验收记录保持原版本，不代表本次修订。
- **下一步：** 无实施待办，用户复核标题对齐即可。

## 上次封装交付 · 0.6.8 入口与收藏流程

- **要求与验收：** R1 星标打开的表单取消/关闭直接收起且无返回箭头；R2 冷启动及重开第一帧不显示首页；R3 普通网页新开、切换、扩展重载后自动显示单个入口；R4 按钮44×44、圆角16、外框20，弧线间距4px，菜单圆角20；R5 已收藏实心星标，增删/换网址及时更新；R6 旧连接自动替换，保留网页和正常菜单草稿；R7 明暗、窄窗、截图、真实Chrome及正式产物分层验收。
- **已定位：** 保存意图在 FLOAT_READY 后才到React；入口只随新页面静态注入，已开页面没有重载后补注入；显示还被首次引导状态限制，旧版本入口失效后只能报刷新。
- **已实现：** 首帧传入保存目标、表单挂载后揭开遮罩；取消/关闭收起，无返回箭头。默认常驻，旧环境退役、健康检查、导航/激活/重载恢复；真实书签驱动实心星标。按钮圆角16、外框和菜单20，4px嵌套间距。
- **实际排错：** Chrome重载复现新旧宿主叠在一起，旧观察器会重新挂回被删除节点；增加跨环境退役事件及旧版兼容停用。正常连接不替换iframe；失效会话重建，关闭动画保持紧凑尺寸，其他未完成操作不会被加载遮罩困住。
- **已验证：** 设计/类型通过，5文件61项针对性回归。10张截图逐张目视，305px无横溢出，短窗360×356操作固定；冷启动36帧/重开10帧均未显示首页。真实Chrome完成新页常驻、跳转、连续重载后网页输入保留、取消/关闭、实心星标增删和对话草稿保留。自建QA书签/对话草稿已清理。
- **最终验证：** 干净源码正式封装通过105文件 / **590项测试**、Node/设计/类型、构建和全部JS语法；后台307,371 bytes。98项构建清单、96个dist/解压/ZIP文件大小和SHA-256一致，源码ZIP与git archive一致。最终从dist在Chrome再次重载0.6.8，直接收藏/关闭和单个有效入口通过。
- **交付：** [扩展ZIP](outputs/Bookmark-Layer-0.6.8.zip)（1,617,581 bytes）、[解压目录](outputs/Bookmark-Layer-0.6.8-unpacked)、[源码ZIP](outputs/Bookmark-Layer-0.6.8-source.zip)（49,917,790 bytes）、[构建清单](outputs/Bookmark-Layer-0.6.8-build.json)、[独立验证](outputs/Bookmark-Layer-0.6.8-verification.json)、[日志](outputs/Bookmark-Layer-0.6.8-checks.txt)。封装提交`ef18f29d35de726e75f6945f331e92f9424b7c20`，树`cc9f38066494b66dbc5577b516f7267ad8e1fd0e`。
- **下一步：** 用户按[10张图集](docs/verification/2026-09-12-entry-recovery/index.html)复核真实入口。本轮实施及本地交付没有待办；专用Chrome测试页已关闭，仅保留图集。后续提交仅回填文档，不覆盖封装源码和旧产物。
- **工作区：** 没有占用中的实施文件；已有5173开发服务保留，本轮4179专用静态验收服务已停止。
- **边界：** 截图是正式UI配隔离开发数据，真实Chrome另有验收记录；本轮未新增联网AI请求，自动增强及字段保护由全量回归覆盖。Chrome内置页/商店等禁止注入页面仍由完整收藏库承接。无新增云同步、跨系统或商店验收；未推送或发布。

## 上次交付 · 0.6.7 大圆角入口与自动 AI 增强

- **要求：** 右侧快捷栏和两个方形按钮采用更大的圆角；打开创建收藏面板即启动真实 AI 增强。
- **实施：** 按钮保持 44 × 44，圆角 20；外框圆角 28，保留磨砂。后台预分析只缓存临时结果；表单显示进度、摘要、标签和失败/保护原因；确认保存后复用结果，未完成的任务继续补入同一条收藏。
- **验收目标：** A1 明暗、悬停和键盘焦点的大圆角截图；A2 打开即分析且不提前写书签；A3 重开/重试/保存不重复分析，不覆盖手动名称、备注和标签；A4 缺少 Key、受保护页面和超时能继续正常收藏；A5 320px/短窗下内容、下拉及操作可达；A6 正式测试、构建、安装态与真实 Provider 分开验收。
- **已实现：** 打开即真实分析；已有结果复用、分析中先保存、原子事务补入 AI 字段并通知列表刷新。页面/来源绑定与隐私检查保留，取消不写收藏。完成结果保存在浏览器会话，15 分钟内有效、最多 32 请求，不另存正文和源网址字段。
- **实际发现并修复：** 原生 Chrome 首轮保存后摘要变化，暴露仅内存缓存不能可靠跨后台休眠的问题；已增加会话缓存和后台重建回归。另修复响应正文不受超时约束、分析在保存中途完成时结果丢失、晚到结果覆盖并发手动修改的风险。
- **验证完成：** 类型与设计约束、4 文件 / 55 项针对性回归通过；构建与产物语法通过，后台 306,299 bytes。13 张截图逐张目视，窄窗305px无横溢出、短窗356px操作固定。图集1150/420px均13/13图片加载、5项勾选可复核。
- **真实 Chrome / AI：** 已从 dist 更新测试构建0.6.7，公开 Example Domain 打开即返回摘要/标签；等待78秒、编辑名称和备注、确认保存后重新打开，181字摘要完全一致、手动字段保留。两个自建测试书签均已移除，其他个人收藏未改。真实后台休眠状态未用调试器检查，后台实例重建由集成测试覆盖。
- **最终验证：** 干净提交正式封装通过 Node/设计/类型、**104文件 / 577项测试**、构建及全部JS语法；98项构建清单大小/哈希一致，96个ZIP/解压/dist文件完全相同，源码ZIP等于对应git archive。原生Chrome再次从dist重载最终0.6.7；设置中确认为DeepSeek / deepseek-v4-flash，没有读取或改动密钥。
- **交付完成：** [扩展ZIP](outputs/Bookmark-Layer-0.6.7.zip)（1,616,693 bytes）、[解压目录](outputs/Bookmark-Layer-0.6.7-unpacked)、[源码ZIP](outputs/Bookmark-Layer-0.6.7-source.zip)（49,583,269 bytes）、[清单](outputs/Bookmark-Layer-0.6.7-build.json)、[独立验证](outputs/Bookmark-Layer-0.6.7-verification.json)、[检查日志](outputs/Bookmark-Layer-0.6.7-checks.txt)。封装源码 `c4f6f8604e2afa5389cc6ff5e4e870effa2b8a97`，树 `92bdf4e6b84f63c5372b73a286df1cf0b60af52c`。
- **下一步：** 用户刷新旧网页即可复核新入口和打开即增强。实施与本地交付没有遗留待办、没有占用文件；本轮只保留图集，临时Chrome测试页完成后关闭；开发服务仍在5173。后续文档提交仅记录验收，不覆盖封装源码或0.6.6等旧版本。
- **边界：** [13张图集](docs/verification/2026-09-11-rounded-ai-save/index.html)为真实UI配隔离测试数据；真实Provider验收单独记录于 installed-ai-checks.json。分析未结束时保存使用可控Provider回归。没有新增云同步、跨系统或商店验收，没有推送发布。

## 上次交付 · 0.6.6 按内容定尺寸的收藏面板

- **本轮要求：** 星标打开的收藏表单过高、过宽、留白过多；根据右侧入口与实际字段定制尺寸。
- **实施计划：** 收藏使用独立的紧凑右侧面板，宽度与收藏/对话工作区分开；高度按真实表单内容计算，超出可视高度时仅正文滚动。收紧来源、字段和推荐文件夹，操作紧跟内容；返回列表恢复用户原有菜单宽度。
- **可验证目标：** C1 常规表单无大块底部留白；C2 320px 窄窗和短窗口内字段/保存按钮可达，无横向滚动；C3 星标首次打开、返回、关闭重开及错误恢复不丢草稿、不提前写书签；C4 长网址、已有收藏与明暗主题经过实际截图检查；C5 真实 Chrome 验证后完成新版本构建与封装。
- **已实现：** 独立 360px 收藏面板，按真实内容自适应高度，最大 640px 且受可视高度约束；字段、来源和推荐项收紧，保存区紧跟正文。目录使用 HeroUI Popover，自动放置，短窗口中的滚动区域按可用高度缩小，Escape 只关闭目录层。
- **已验证：** 11 张截图逐张目视；普通 360 × 450px、提示 499px、已有收藏 530px、长 URL 551px；窄窗实际宽 305px，无横溢出。短窗口正文滚动 0→143，底部坐标不变，自制无底轨滚动条渐隐。原生 Chrome 冷开/重开、提示全文、URL 展开、返回 449px 原工作区、下拉 Escape 已检查，未写入个人收藏。
- **已解决补充问题：** 入场动画污染尺寸测量；正式 CSS 顺序覆盖标题/底部间距；旧目录浮层被表单裁剪及 Escape 关闭整张表单。相关类型、设计和回归测试通过。
- **最终验证：** 干净提交执行正式封装，Node、设计 token、TypeScript、**103 文件 / 556 项测试**、构建与全部 JavaScript 产物语法通过，后台 300,901 bytes。封装后原生 Chrome 再次从 dist 重载 0.6.6，原 FOLD 页的紧凑表单、提示全文和目录 Escape 正常；未增删或更新个人书签。
- **交付完成：** [解压目录](outputs/Bookmark-Layer-0.6.6-unpacked)、[扩展 ZIP](outputs/Bookmark-Layer-0.6.6.zip)（1,613,921 bytes）、[源码 ZIP](outputs/Bookmark-Layer-0.6.6-source.zip)（48,727,910 bytes）、[构建清单](outputs/Bookmark-Layer-0.6.6-build.json)、[独立核验](outputs/Bookmark-Layer-0.6.6-verification.json)、[完整检查日志](outputs/Bookmark-Layer-0.6.6-checks.txt)。干净源码 `9ccd1a5e04d6d51f9c53f95deb728483bc7e0c00`；98 项清单大小/哈希、96 个 ZIP/dist/解压/安装构建文件一致，源码 ZIP 与对应 git archive 相同。
- **当前下一步：** 用户直接在原 FOLD 页检查紧凑表单，其他已打开网页刷新一次；[11 张图集](docs/verification/2026-09-11-compact-save/index.html)提供可勾选复核，1150/420px 均 11/11 图片加载且无横溢出。本轮本地实施、封装和安装核验已无待办，没有占用文件。隔离浏览器已结束，只保留图集；所有临时扩展详情页已关闭，开发服务仍在 5173。仅后续文档提交回填结果，不覆盖封装源码和旧版本。
- **边界：** 真实 Chrome 检查入口、尺寸、来源与返回/下拉层级；窄短窗、已有收藏与保存错误用隔离开发数据和正式控制器测试。没有新增真实 AI、云同步或跨系统测试；没有推送和发布。先前 0.6.5 的实际保存验证不冒充本轮重新写入验收。

## 上次交付 · 0.6.5 方形磨砂快捷栏与收藏表单

- **本轮三项批注均已实现：** 两个按钮 44 × 44px、图标居中；52 × 100px 收起快捷栏使用半透明磨砂，减少透明效果/强制颜色有实色回退。完整收藏库重排品牌、双导航、右侧工具和搜索筛选。星标展开网页内菜单并打开真实收藏表单，确认前不写书签。
- **可靠性补充：** 打开意图保留至 React 与来源标签就绪，初始视图消息不会吞掉星标请求，重复请求保留草稿。读取最新备注后显示已有收藏，避免首次加载时覆盖备注。保存错误在表单内显示，重试保留字段；标题和底部操作固定，正文独立滚动。
- **视觉验收：** [19 张本轮截图与电脑端复核](docs/verification/2026-09-11-save-entry-header/index.html)；逐张经图像工具查看，SHA-256 见同目录 visual-review.json。1440/768/320px 收藏库 × 明暗六组无横溢出；400px 菜单和 320px 宿主窗口（305px 可用宽）表单四组无横溢出，备注在调整尺寸/主题后保留。按钮两个主题都 44 × 44px，图标居中偏差 0；透明底图与实色回退已检查。[逐项目标与证据](docs/verification/2026-09-11-save-entry-header/README.md)。
- **真实 Chrome：** 原扩展已从本项目 dist 重载为 0.6.5，原生界面核验首次星标、取消、暖启动、确认保存，以及刷新后已有收藏的名称、备注、文件夹保留。独立公开测试书签已移除并确认工具栏恢复添加状态，其他书签未改动；新增 QA 标签已关闭。正式收藏库新顶部亦已目视核验。95 个被测构建文件的哈希与最终封装逐一相同。
- **最终检查：** 干净源码执行正式封装，Node、设计 token、TypeScript、**102 文件 / 549 项测试**、构建及全部 JavaScript 产物语法通过，后台 300,901 bytes。包括 39 项关联专项。已修正视觉检查发现的导航文字偏上（组件实际为 role=tab 的 div）和全页表单继承弹窗 padding 的问题；四条旧顶栏断言同步为当前设计后完整回归通过。
- **封装完成：** [解压目录](outputs/Bookmark-Layer-0.6.5-unpacked)、[扩展 ZIP](outputs/Bookmark-Layer-0.6.5.zip)（1,612,172 bytes）、[源码 ZIP](outputs/Bookmark-Layer-0.6.5-source.zip)（47,644,669 bytes）、[构建清单](outputs/Bookmark-Layer-0.6.5-build.json)、[独立核对](outputs/Bookmark-Layer-0.6.5-verification.json)、[完整检查日志](outputs/Bookmark-Layer-0.6.5-checks.txt)。干净源码提交 `c9601447b756375a5c9331cd7347f333e688a3c5`；97 项大小/哈希、95 个解压文件与 ZIP / dist / 安装态构建记录全部一致，源码 ZIP 与对应 git archive 完全一致，公开资源存在。
- **下一步：** 用户刷新普通网页与完整收藏库页面，即可复核三项调整。图集 1150/420px 下均 19/19 图片正常且无横溢出；自制细条滚动时出现、静止渐隐，正文与操作栏位置不变。本轮已无本地实施或封装待办，没有占用文件。验收浏览器已清理并保留图集，开发服务仍在 5173。随后仅文档提交回填记录，不覆盖已封装源码和任何旧版本产物。
- **边界：** 截图使用明示的隔离开发数据；真实 Chrome 单独记录。未新增真实 AI 对话、生产云同步/恢复、其他系统或商店验收，没有推送或发布。

## 上次交付 · 0.6.4（历史记录）

## 0.6.4 右侧快捷栏与产品精简

- **本轮需求：** 按六张最新标注替换旧球形入口。右侧贴边双按钮 bar（展开 / 快捷收藏）连续变形成网页内菜单；只有左边可调宽，去掉角落拖拽标记。保留单一收藏/对话工作区及底部常驻输入框。
- **已完成：** 44 × 96px 右侧 bar；菜单默认 400px、可调 320～640px，高度随可视窗口，上下各留 12px；更窄窗口保持在可用区域内。展开/收起 280/210ms，支持减少动态效果与快速反向开合，保留原 iframe 和草稿。只把自己的浮层置前，修复 NexAlign 被动入口遮住输入框的问题。
- **真实快捷收藏：** 复用正式保存流程，延后后台增强/同步以免入口等待；绑定当前标签、文档和宿主身份，合并连点请求，重复收藏不修改原数据，错误可重试。0.6.3 的 iframe 身份挑战及 5/15 秒超时保留。
- **设置与精简：** 桌面设置改为分类导航与内容面板，窄窗使用共享 HeroUI 分类选择器；六组表单保留挂载，切换不丢编辑内容。移除整理提案、报告、重新发现三页及通知、角标入口；主导航仅收藏库/主题图谱，旧 URL 回到收藏库并保留搜索条件。兼容历史数据和对话内部动作，未删除用户已有数据。
- **AI 自适应：** 长引用全文换行，清除造成最小宽度膨胀的布局约束。代码自动换行；窄表格转为带字段名的条目，多于四列的表格始终转条目。替代之前的块内横滚方案，任何正文块均无需左右拖动。
- **可验证目标：** G1 双按钮可用；G2 连续开合及草稿保留；G3 只调宽且视口内可达；G4 收藏来源正确、防重复与恢复；G5 设置分组不丢输入；G6 仅两项主导航及旧链接兼容；G7 多宽度全文不横滚；G8 视觉、真实 Chrome、封装独立核验。[逐项目标及证据](docs/verification/2026-09-11-edge-workspace/README.md)。
- **实际 Chrome：** 原扩展 `ppjmhonejgpcdmjmcbbdjookgiagambm` 从本项目 `dist` 更新 0.6.4。双按钮显示、真实收藏/重复提示、菜单正常打开均通过；直接打开用户原来的动效历史回答，在 320px 下检查完整长引用，没有重新请求 AI。真实菜单宽度恢复 400px。公开 QA 页新增的测试书签已移除，其他收藏未修改。曾遇浏览器调试工具干预导致 iframe 空白；终止该连接、刷新后使用原生 Chrome 界面继续验证，不将其算作正常产品流程。
- **视觉验证：** [31 张最终截图与电脑端勾选验收](docs/verification/2026-09-11-edge-workspace/index.html)，每张通过图像工具单图或标记对照图检查，SHA-256 见 `visual-review.json`。全部为正式 UI 的隔离 DEV 画面，避免存入个人浏览数据。320/400/640px × 明暗六组回答宽度等于滚动宽度，全部正文块无横溢出，全文均为 717 字符。设置桌面 800 × 620，320px 窗口为 288 × 608；只有当前分组可见，无横溢出。图集在 1150/420px 下 31/31 图片加载正常、无横溢出。
- **交互验证：** 浏览器实际鼠标从左边拖动 400→640，菜单高度保持；快速收起重开保留同一 iframe/中文草稿，减少动态效果时无动画。实际窗口 420 × 640 时，扣除网页滚动条后菜单为 405 × 616，保持在可视区内。主题切换中间帧不作为稳定截图，已重新拍摄并等待过渡完成。
- **自动验证：** Node、设计约束、TypeScript、102 文件/544 项测试、构建及产物 JavaScript 语法通过；后台 300,901 bytes（硬门 330,000）。含宿主启动/恢复、来源身份、并发收藏、Markdown 全文/表头、设置编辑保留及三条旧路由回归。正式封装已从干净提交再次重跑并通过完整检查。
- **交付完成：** [解压目录](outputs/Bookmark-Layer-0.6.4-unpacked)、[扩展 ZIP](outputs/Bookmark-Layer-0.6.4.zip)（1,611,226 bytes）、[源码 ZIP](outputs/Bookmark-Layer-0.6.4-source.zip)（45,620,683 bytes）、[构建清单](outputs/Bookmark-Layer-0.6.4-build.json)、[独立核对](outputs/Bookmark-Layer-0.6.4-verification.json)、[正式检查日志](outputs/Bookmark-Layer-0.6.4-checks.txt)。干净源码提交 `ee057e01d758aa908b65820ca7686c48dc237d44`；97 项大小/SHA-256、95 个解压文件与 ZIP、当前 `dist` 及对应提交的源码 archive 全部一致，公开 iframe 资源完整。
- **当前下一步：** 用户刷新已打开的普通网页后，按新图集复核入口、宽度、直接对话和分组设置。本轮本地实施、视觉和封装没有遗留待办；没有占用中的实施文件。只做本地提交，未推送、部署或发布。后续文档提交仅回填交付记录，不覆盖已封装源码和产物。验收浏览器已清理并保留图集，本地开发服务仍在 5173 端口。
- **验证边界：** 真实 Chrome 核验菜单、快捷收藏与已有回答，管理设置的分组和输入保留在隔离浏览器与集成测试验证。新增真实 Provider 对话、生产云同步/恢复、其他系统和商店发布尚未验收。原 0.6.3/0.6.2 封装不覆盖。

## 上次交付（0.6.3，历史记录）

## 0.6.3 菜单启动修复（历史交付）

- **用户故障：** 点击悬浮球后长时间显示“正在打开收藏”。已在用户实际 Chrome 的 0.6.2（加载自本项目 `dist`）复现，菜单内部收到“此菜单已失效”。
- **已确认原因：** 安装态的 `webNavigation.getAllFrames/getFrame` 不返回扩展自己的 iframe；`runtime.getContexts` 可返回该 iframe。旧身份校验使用前者，误拒绝正常菜单。另有首次连接失败时尚未建立桥接上下文，错误消息无法通知外层加载遮罩的问题。
- **已解决：** 改用扩展上下文查验，并通过顶层宿主向其实际持有的 iframe 发挑战、由浏览器提供回复者身份来确认归属；保留网页、会话、文档及帧绑定。首次连接失败也能立即通知外层显示原因与重试入口；挑战 5 秒、加载 15 秒有界超时。
- **实际 Chrome：** 本机原有扩展已加载 0.6.3，从同一 `dist` 更新。公开原网页刷新后，点击首个界面读取（1196 ms 内）显示真实收藏树与底部输入框；搜索、更多→设置、收起重开及中文草稿保留均验证通过。只读后台确认菜单已绑定，而网页框架列表仍没有该 iframe；启动后台无错误。验证草稿已清除，没有发送 AI 问题或增删收藏。
- **自动验证：** 干净源码执行正式封装，100 文件/537 项通过；含 40 项启动/身份/恢复专项。Node、设计约束、类型检查、构建与全部产物 JavaScript 语法通过；后台 299,806 bytes。[本轮记录与脱敏证据](docs/verification/2026-09-11-startup-recovery/README.md)。
- **交付与独立校验：** [解压目录](outputs/Bookmark-Layer-0.6.3-unpacked)、[扩展 ZIP](outputs/Bookmark-Layer-0.6.3.zip)、[源码 ZIP](outputs/Bookmark-Layer-0.6.3-source.zip)、[构建清单](outputs/Bookmark-Layer-0.6.3-build.json)。对应干净提交 `edba4d69822a64d581091b5bad47cd6c6cb2be43`；97 项大小/SHA-256、95 个解压文件与 ZIP、源码与 git archive 全部一致。当前加载的 `dist` 95 个文件与封装目录逐字节一致，封装后再次重载扩展并刷新原网页，1245 ms 内首个界面读取确认正常。
- **后续边界：** 本次启动修复已完成。已打开的其他旧网页需刷新一次替换旧脚本。真实 AI 服务、云同步、其他系统/最低 Chrome 版本及商店发布不属于本次已验证结果。
- **工作区：** 没有占用中的实施文件或未完成的本地封装工作；本轮只有本地提交，未推送或发布。随后文档提交只补充交付记录，不覆盖已封装源码和产物。

## 前轮交付记录（0.6.2，以下不是本次启动修复结论）

## 前轮交付 · 0.6.2 单一对话工作区（历史记录）

用户对 0.6.1 提出的 14 条批注已经逐项处理。**0.6.2 的交互与视觉记录见本节和 [0.6.2 图集](docs/verification/2026-09-09-conversation-workspace/index.html)**；启动与实际 Chrome 状态以上方 0.6.3 记录为准。历史版本的“视觉通过”不代表用户认可，也不能覆盖当前方向。[批注对账与详细证据](docs/verification/2026-09-09-conversation-workspace/README.md) 保留每项可复查目标。

### 已完成与已解决问题

- **结构重做：** 悬浮菜单移除收藏 / AI / 设置 Tabs，只有一个工作区和常驻底部的大输入框。直接发送进入当前对话；返回收藏、继续追问和打开设置时沿用同一输入框。设置与历史使用次级窗口，新会话放进更多菜单；没有独立 AI 欢迎页或第二条对话工具栏。收藏当前网页移为标题栏星标，仍调用正式收藏 / 编辑控制器。
- **共享视觉：** 继续使用 HeroUI 3.2.4。搜索、筛选、按钮、标签改为小圆角；搜索清除内部重复边框与阴影；字段标签独立成行，保持 8px 间隔，下拉箭头留 12px。清除粗选中环，键盘焦点保留细单线；日夜开关拇指均为亮色。收紧设置重复留白，主题图标归入顶部工具组；删除确认改为上方说明、下方同排操作。
- **内容与滚动：** 320 / 400 / 560px 的 AI 正文不横溢出，宽表格和代码只在所属块内滚动。所有菜单滚动区使用共享自制悬浮细条，透明底轨，闲置渐隐，不占用正文宽度；多行输入达到高度上限后内部滚动，底部操作始终可达。
- **对话连续性：** 未配置服务也能先输入，发送打开真实设置并保留问题。停止后迟到结果不会覆盖已取消状态，历史与连续追问继续使用正式数据流。额外修复流式内容已返回却仍只显示“思考中”的问题；提案摘要直接显示具体操作，问题编辑改为旁侧图标。
- **慢加载恢复：** 实际宿主回放发现：菜单超时后迟到的合法就绪消息没有清除重试标志，导致下一次展开重建 iframe。现已修复；慢加载恢复后保留原会话，真正失败仍可重试，废弃 iframe 的迟到消息不会影响新菜单。已有修复前失败及修复后通过的回归证据。

### 验证结果

- **实际画面：61 张产品截图逐张经图像工具检查。** 覆盖菜单、直接对话、流式生成、设置、下拉、长备注、删除确认、长历史和输入、完整收藏库、实际宿主及滚动过程。菜单 17 组与管理页 21 组量测无页面横溢出或缺失自制滚动条；选中值垂直中心偏差小于 0.01px，选中标记偏差 0。
- **真实浏览器交互：** 拖动滚动条使偏移 280→552px，细条 5px / 命中区 12px，内容宽度保持 400px；实际 160ms 渐隐的 40ms 帧透明度 0.39278，最终为 0。宿主在 1280×900px 明暗主题和 420×760px 窄窗无越界；收起 / 重开后为同一 iframe、同一 composer、同一中文草稿，焦点回到输入框，原网页宽度仍为 1265px。
- **图集：** 本地文件页在 1150px 与 420px 下均 61/61 图片加载成功，无横向溢出；额外两张图集截图保留在 logs，不计入 61 张产品画面。旧图集默认折叠并指向当前版本。逐图 SHA-256 与审核记录一致。
- **完整检查：** 干净源码执行 `npm run package:artifacts`，**99 个测试文件 / 524 项通过**；Node、设计约束、TypeScript、构建与全部生产 JavaScript 语法检查通过。包含 4 个根应用对话集成用例、2 个宿主恢复用例；旧文字按钮断言已更新为当前图标按钮的可访问名称，正式处理器检查继续保留。
- **产物：** `npm run verify:artifacts` 通过；独立复核构建清单 97 项的字节数和 SHA-256，95 个解压文件与扩展 ZIP 逐个相符；源码 ZIP 与对应提交的 `git archive` 完全相符，公开 iframe 资源无缺失。后台文件 298,658 bytes，低于 330,000-byte 硬门。

### 0.6.2 交付与下一步

- **加载目录与文件：** [0.6.2 解压目录](outputs/Bookmark-Layer-0.6.2-unpacked)、[扩展 ZIP](outputs/Bookmark-Layer-0.6.2.zip)（1,611,275 bytes）、[源码 ZIP](outputs/Bookmark-Layer-0.6.2-source.zip)（42,621,368 bytes）、[构建清单](outputs/Bookmark-Layer-0.6.2-build.json)、[独立校验](outputs/Bookmark-Layer-0.6.2-verification.json)、[完整检查日志](outputs/Bookmark-Layer-0.6.2-checks.txt)。北京时间 2026-09-09 20:33:46 构建，20:33:49 封装；0.6.0 / 0.6.1 产物没有覆盖。
- **身份：** 干净提交 `b4b033d3cbd2e10f0a1464be1975d1139d741f54`，包含产品改造提交 `2c8fa59` 和最后的测试断言修订。随后只补充交付文档，已封装产品源码保持不变。分支为 `codex/aarre-floating-ui-20260909`，没有推送、合并、部署或商店发布。
- **本轮完成：** 本地 UI、功能回放和封装没有待办，也没有占用中的实施文件。最新验收页已保留；本地开发预览为 `http://127.0.0.1:5173`。不要重新执行整套审计和改造，下一步是用户电脑端按图集复核实际安装态。
- **外部边界仍开放：** 上述浏览器回放使用明示的隔离 DEV 收藏和 QA 会话，没有写入用户 Chrome 书签或请求真实 AI 服务。真实 Chrome 的加载版本 / Service Worker、原生收藏写操作、实际 Provider、正式恢复和截图仍需安装态验收。服务端保持 0.1.15，迁移 009 未部署；生产云端、双客户端和历史兼容 / 规模 / 商店素材门槛不能据本轮 UI 完成而视为通过。
- **电脑端操作：** 使用新的 0.6.2 解压目录更新扩展，刷新普通网页后打开悬浮菜单，按最新图集中的勾选项验证直接对话、连接服务、停止 / 追问、编辑删除、滚动、明暗主题和完整收藏库。

## 前轮交付 · 0.6.1（用户已要求修订）

**以下为 0.6.1 的历史交付记录，不是当前状态。** [0.6.1 图集与电脑端验收](docs/verification/2026-09-09-ui-refinement/index.html)、[完整证据记录](docs/verification/2026-09-09-ui-refinement/README.md) 和 [逐图审核清单](docs/verification/2026-09-09-ui-refinement/visual-review.json) 配套使用。下方 0.6.0 为前轮交付事实，不能覆盖本节。
...
**以下为 0.6.0 的历史交付记录，不是当前状态。** [实际截图与可勾选验收页](docs/verification/2026-09-09/index.html)、[完整验收对账](docs/verification/2026-09-09/README.md)、[统一实施计划](docs/FLOATING_UI_REMEDIATION_PLAN.md) 和 [原始审计](docs/AUDIT_2026-09-09.md) 配套使用。下方历史记录不能覆盖本节。
