...
**Aarre 0.3.4 的六批整改正在按 `docs/REMEDIATION.md` 逐批执行。** 第 1–4 批已完成代码、自动化和开发预览验证；第 4 批的分类封面兜底率虽从 59.67% 降至 57.33%，但没有达到 PRD 的 ≤12% 硬指标，原因和继续投入成本已如实记录。M1 的 F1–F6 / F22、M2 的 F7–F13、M3 本地项 F15–F17、M4 本地项 F18–F20 均已有正式实现；F8 按 PRD 第 12 章只实现已决策的第一层强词法检索。按用户要求，F14 自建云端与依赖云端的 F21 Web/PWA 不在本轮范围，现有云端文件未顺手改动。
...
**下一轮要做的事全部写在 `docs/REMEDIATION.md`。** 那是一份基于独立代码审查产出的六批次整改清单，每条都带验收标准，接手的 Agent 从它读起，不要自行另立计划。
