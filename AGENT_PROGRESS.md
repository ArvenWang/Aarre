...
最后更新：2026-08-03（0.5.60 / 修复资产 ID 与内容不匹配导致的同步 409）
...
**当前工作区最新状态：0.5.56。** 近期链路：0.5.56 删除卡片改为“局部移除”（`onChanged` 上报 `removed`，ManagerApp 直接过滤列表，不再整页刷新），站点图标恢复强制从云端拉回（本地无当前渲染版本图标时不再被 state 哈希跳过）；0.5.55 编辑/删除刷新跳过全量同步 + 封面会话级缓存；0.5.54 删除保持滚动位置（`preventScroll`）、favicon 质量门槛放宽（128px/1.2/0.15 → 16px/3:1/0.01，`SITE_ICON_RENDER_VERSION` 6→7）、Google 账号区块移到设置页顶部；0.5.53 清理“同一截图绑定多个网址”的 29 条云端错误快照（服务器已软删除，坏组归零）+ 客户端启动自动清理本地重复快照；0.5.52 AI 对话格式失败诊断留痕 + JSON 解析容错扩展（注释/尾逗号/字面换行）；0.5.51 AI 对话修复（JSON 字符串内字面换行修复、思考阶段单次尝试即降级）；0.5.50 同一网址多条收藏根因修复（确定性 bookmarkItemId + 同步去重，云端 740→~265 自动清理）。服务器 0.1.10 已上线（允许 GIF 资产上传，release `20260803-gif-v28`）。当前 `dist/` 为 cloud-enabled 0.5.56 正式构建（带 key）。F14 生产 API、数据库、COS/CAM、Google Web OAuth、DNS/TLS、定时备份、两分钟健康巡检、独立 GlitchTip project 与含 SSH Key 的加密恢复包已经部署；Google 品牌审核、卸载重装恢复和正式 Web Store ID 尚未完成。
