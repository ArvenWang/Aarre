...
**Aarre 0.3.7 已完成 `docs/REMEDIATION.md` v2 的第 1–7 批整改，并修复悬浮预览在同一条目内移动时闪退的问题。** 第 7 批补齐 `/favicon.ico` 多帧解析与约定路径来源，把墨迹覆盖率的统计范围改为图标实际绘制区域；同一时刻、同一书签源的 300 条样本对照从 **51%（153 条）降到 46.67%（140 条）**，净减少 13 条分类兜底，最大缩放比仍为 1。PRD 的 ≤12% 指标仍不可达：当前 140 条兜底中 28 条有质量候选但被拒、112 条没有合格候选，后续是否调整指标或扩大资产来源需要产品决策。M1 的 F1–F6 / F22、M2 的 F7–F13、M3 本地项 F15–F17、M4 本地项 F18–F20 均已有正式实现；F8 按 PRD 第 12 章只实现已决策的第一层强词法检索。按用户要求，F14 自建云端与依赖云端的 F21 Web/PWA 不在本轮范围，现有云端文件未顺手改动。
