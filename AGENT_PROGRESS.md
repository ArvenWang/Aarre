...
最后更新：2026-09-09（0.6.1 视觉整改、逐图审核、完整检查与可追溯封装均已完成；安装态与真实外部服务验收仍开放）

## 当前进展 · 0.6.1 视觉细节复核

**这是当前项目事实与接力入口。** [0.6.1 图集与电脑端验收](docs/verification/2026-09-09-ui-refinement/index.html)、[完整证据记录](docs/verification/2026-09-09-ui-refinement/README.md) 和 [逐图审核清单](docs/verification/2026-09-09-ui-refinement/visual-review.json) 配套使用。下方 0.6.0 为前轮交付事实，不能覆盖本节。

### 用户要求与实施范围

- 用户已授权连续完成：所有菜单滚动条自制、悬浮、无底轨、闲置渐隐；下拉箭头和组件内容留足边距；组件与文字比例匹配，并要求实际视觉复核。
- 从干净的 `9e4fa70` 开始，沿 `codex/aarre-floating-ui-20260909` 实施；扩展、package 和 Manifest 递增至 **0.6.1**。没有推送、合并、部署或商店发布。
- 继续采用 **HeroUI 3.2.4**。对照 NexAlign（`LayerScope`）的滚动与控件规范及存档截图；其工作区有其他任务改动，仅只读参考，未改动或宣称重新运行过该项目。

### 已完成与已解决问题

1. **统一滚动体验：** 所有滚动入口接入共享双轴悬浮组件，删除两套重复实现；可拖动、可用方向键及 Home/End，支持尺寸/内容变化、焦点和拖动取消。5px 细条、12px 透明命中区，无底轨、不挤正文，约 850ms 闲置后以 160ms 淡出；缩放柄和双轴交点保留空间。
2. **组件比例与留白：** 下拉箭头距边框内侧 12px，箭头与选中标记进入正常布局，选中值统一 13px；修正旧弹出宽度变量。常规控件 32–40px / 13–14px 文字，紧凑辅助文字最低 12px，Portal 同步紧凑密度。
3. **编辑与长内容：** 菜单和管理页编辑都固定标题/操作区，长正文独立滚动；管理编辑说明到首字段实测 20px。修复输入框重复 padding、AI 入口过高、历史摘要缩进与 Markdown 混杂。宽表格/代码不再撑宽 AI 正文，改为各块内横向滚动。
4. **管理页细节：** 搜索框重复阴影、窄窗主题按钮独占一行、补齐封面按钮文字贴边已修复；设置字段样式归入页面自身共享样式，管理窗口里标签在字段上方并留 8px 间隔。
5. **弹层稳定性：** 移除影响直接子项排版的中间包装，保留 HeroUI 焦点管理；全局设置/备份 Portal 移到 Tabs 集合外，解决直接链接重复创建两个窗口、互相屏蔽点击的问题。直接打开、关闭、重开均只有一个有效窗口。
6. **可靠的视觉取证：** Vite 日志证实保存验收 JSON 会刷新预览，已限定 Tailwind 源码范围并排除证据/产物目录监听。3 秒写入探针确认导航时间不变。中间空白图在 logs 保留，不计入最终视觉结果。

### 验证情况

- **51 张最终截图逐张经图像工具查看。** 覆盖收藏/AI/设置菜单、模型和文件夹选择、长备注与宽表格、历史会话、五个管理功能页、管理设置/编辑/删除确认、备份、封面补齐确认、引导及隐私；明暗主题，320/400/560px 菜单与窄窗管理界面。图片哈希和审核重点均在 `visual-review.json`。
- 菜单 12 个与管理页 15 个量测场景无页面横溢出，实际滚动区都有共享滚动条。后续弹层修正以 `final-dialog-checks.json`、`shared-settings-fields.json` 优先。
- 实际鼠标拖动：偏移 **280→537.5px**，细条 5px / 命中区 12px。渐隐过渡中的 40ms 帧透明度 **0.39278**，恢复播放后为 **0**；正文宽度全程 **384px**，底轨透明。
- 400px 窗口的 AI 正文宽/滚动宽均 **384px**；表格与代码容器 **352px**，分别容纳 **708px / 1134px** 内容，键盘 End 将表格横向移到末端。320px 窗口的正文宽/滚动宽均 **304px**。
- 最终管理编辑复核：首字段间距 **20px**，420px 窗口底部操作区终点 **807px < 820px**；320px 删除确认终点 **627px < 640px**，所有按钮可见。Tab 焦点保持在窗口内，Escape 先退出确认再关闭编辑。备份直接入口与重开均为 **1 个窗口，非 inert**。
- 正式 `npm run package:artifacts` 从干净源码重新执行全部检查：**97 文件 / 518 项通过**，Node、设计约束、TypeScript、构建及全部生产 JavaScript 语法检查通过。`npm run verify:artifacts` 通过；后台 **298,658 bytes**，低于 330,000-byte 硬门。
- 图集实际打开后 **56/56 张图片加载成功**（51 张最终图 + 5 张参考/问题图），没有横向溢出；最终 51 张图片的哈希与逐图审核清单一致。

### 0.6.1 交付记录与外部边界

- **正式交付完成：** [解压加载目录](outputs/Bookmark-Layer-0.6.1-unpacked)、[扩展 ZIP](outputs/Bookmark-Layer-0.6.1.zip)（1,603,140 bytes）、[源码 ZIP](outputs/Bookmark-Layer-0.6.1-source.zip)（38,182,001 bytes）与 [构建清单](outputs/Bookmark-Layer-0.6.1-build.json)。北京时间 2026-09-09 18:38:20 构建，18:38:23 封装；没有覆盖 0.6.0 交付物。
- **源码身份：** 干净提交 `d09373638e3f5d27b459663a0a5672b949acd108`（`fix: refine floating scrollbars and UI details (0.6.1)`）。随后仅补充本文件和证据 README 的交付记录，已封装的产品源码保持不变。
- **独立校验：** 构建清单 **96 个条目**逐文件大小/SHA-256 相符；ZIP、解压目录及源码包版本均为 0.6.1，源码包注释提交身份相符；iframe 公开资源无缺失。结果在 [派生校验记录](outputs/Bookmark-Layer-0.6.1-verification.json)，完整检查日志在 [检查日志](outputs/Bookmark-Layer-0.6.1-checks.txt)。
- 本地实施、视觉和封装没有遗留待办，也没有占用中的文件。Ego 已完成任务空间清理并保留图集页面，`http://127.0.0.1:5173` 的本地预览保留供复核；下一步进入下列安装态和真实服务验收。
- 预览使用明示的隔离数据（309 条收藏、22 个 QA 会话、长备注），未写入用户的 Chrome 数据或调用真实 AI。
- 已安装 Chrome 的实际版本/路径、真实 Provider/云端、原生 Chrome 写操作仍未在本轮验收；用户回电脑后按图集清单加载 0.6.1 复核。
- 服务端保持前轮 **0.1.15 / 迁移 009 尚未部署**。此前兼容性、规模、正式恢复/截图和商店素材的外部门槛继续有效；不能将本轮本地 UI 完成解释为商业化发布全部门槛通过。

## 前轮交付 · 0.6.0

**这是当前项目事实与接力入口。** [实际截图与可勾选验收页](docs/verification/2026-09-09/index.html)、[完整验收对账](docs/verification/2026-09-09/README.md)、[统一实施计划](docs/FLOATING_UI_REMEDIATION_PLAN.md) 和 [原始审计](docs/AUDIT_2026-09-09.md) 配套使用。下方历史记录不能覆盖本节。

- **用户授权：** 已要求制定可验证的功能、体验与视觉目标，并连续完成实施；用户使用手机，回到电脑后亲自验收。
- **Git：** 开工时工作区干净；已 fetch、fast-forward pull。结束前再次 fetch，`origin/main` 仍为 `cc352cfb16b30fb16b84eec9dff74aa1ea23b8b8`。实施位于 `codex/aarre-floating-ui-20260909`，没有推送或合并主线。
- **版本：** 扩展与 Manifest **0.6.0**；服务端 **0.1.15**。当前 `dist` 已构建。完整产物提交身份和哈希由正式打包清单记录；封装结果见下方交付记录。
- **计划：** M01～M07 的代码、迁移、UI 和文档已实施；M08 的本地检查、实际截图审核已完成，安装态和真实外部服务部分仍开放。**不能宣称商业化发布的所有门槛已通过。**
- **形态：** 已采用 HeroUI 3.2.4 和「悬浮球 + 收藏/AI/设置菜单 + 完整收藏库」。NexAlign 参考项目是 `LayerScope`，核对提交 `2a7bd4ac0643010d82021b8877f720bcdcecfb82`；未修改、安装或验证该项目运行态。

### 已完成与已解决问题

- **同步正确性（A01/A02/A05/A06/A10）：** 数据、分页游标、资源版本在同一事务提交；删除不被旧更新复活；合并迟到响应保留新备注；字段清空具有明确协议；图片的字节、版本与页面绑定一起核对。账号切换后旧请求不重试到新账号。
- **异常恢复（A03/A04/A07/A08/A09）：** 部分失败不再显示全部完成；手动重试能发出真实请求并尊重服务端限流；临时刷新失败保留有效登录；刷新重试幂等；AI 断连/静默/总时长有结束边界，部分回复和会话可恢复；最近动作可继续加载并撤销第 13 条以后记录。
- **备份与隐私（A11/A18）：** 登录仅连接账号；启用完整备份必须明确同意且绑定当前账号。图片上传的独立入口也验证同意。archive v2 包含完整原生目录、图片、备注和稳定会话，整文件及逐图校验；恢复到独立目录并重映射 ID，支持重复识别、中断续接和结束恢复。凭证、授权和旧动作不恢复。
- **服务端：** 候选图片经过对象校验后再原子切换；刷新凭证恢复窗口、上传候选与清理任务进入迁移 `009_recoverable_transfers.sql`。代码与本地数据库集成已通过，**尚未部署**。
- **功能入口：** 普通网页上的 52px 球、四角拖动、菜单缩放、位置保存、站点/全局开关、快捷键和右键入口已落地；来源标签/文档/iframe/nonce 校验、截图隐藏握手、页面重建与加载失败重试已接入。Chrome 受限页面退到完整收藏库。Side Panel 主入口与权限已移除。
- **UI 与体验（A16/A17）：** 全套基础控件走 HeroUI，保留原生树、卡片和业务图谱。日夜主题、正文/辅助/警示色、危险按钮、窄窗导航、编辑固定操作区、弹层居中、开关名称、中文草稿、AI 初始状态和减少动态效果已修正。图谱有键盘可用的主题列表，图形可见文字不再过小过淡。
- **验证与文档（A12/A13/A14/A15/A19）：** 修复 DEV 事件接口、过期时钟 fixture、审计跳过假绿；两端依赖审计为零报告漏洞；README、引导与本地/服务端隐私文案已同步。原审计的 12 张截图和 10 个失败复现保留不动。
...
- 两端 npm audit 均 **0** 个报告漏洞。已有商店素材仅完成格式检查，旧营销素材尚未改成新形态。
- **实际视觉：36 张最终截图均经图像工具逐张查看。** 收藏/AI/设置/五个管理页有明暗配对；包含四角、360/420px 窄窗、编辑、删除、备份预览、引导和隐私页。全部来自明确标注的 DEV 数据场景，不能算已安装 Chrome。
- **实际交互：** 四角鼠标拖动与 400×600 → 460×680 指针缩放通过；360/420/1280/1440px 几何检查无越界；20 次开关只有一个宿主、同一 iframe；页面删除宿主后重建新的 iframe 并加载成功；18 次 Tab 留在编辑弹层，Escape 分层退出。
- **可读性：** 30 组信息色/背景与两组危险按钮文字比值均 ≥4.5；最低 4.59；导航图标中心误差 0px。减少动态效果下，球过渡 0s，控件过渡 0.01ms。
- **性能：** 309 条 DEV 收藏，暖展开 p95 **57.1ms**，20 次搜索 DOM 更新 p95 **172.3ms**。实际显示帧实验出现约一秒延迟，而普通计时器正常；不能把 DOM 耗时当作显示完成。解锁后的真实 Chrome 与 1,000/2,000 条规模仍需验证。

### 未解决的外部验收与下一步

1. **已安装 Chrome：** Mac 锁屏，Ego 不支持扩展管理协议；未核对实际加载路径、版本、Service Worker/Errors。用户回电脑后按验收页做原生收藏/编辑/移动/删除/撤销、跨标签/窗口、页面跳转和刷新。
2. **真实 AI 与云端：** 服务端 0.1.15 和迁移 009 未部署；真实 Provider、双客户端离线/重连、COS 图片恢复未联调。新云端恢复/候选图片协议要先升级服务端再执行发布验收。
3. **安装态恢复与截图：** archive 的实际空 Chrome 配置恢复、正式单页/批量封面中排除球/菜单的逐图检查仍需执行。现有图片是宿主隐藏场景截图，不能替代正式生成封面。
4. **兼容与规模：** Chrome 最低 134/当前稳定版、Windows、系统输入法/读屏、BFCache/休眠、1,000/2,000 条规模、真实显示帧与内存尚未完整覆盖。
5. **发布与素材：** 用户现场视觉/体验验收完成后再判断发布条件；本轮没有推送、部署或商店发布。新营销截图和视频需在正式安装态确认后制作。
...
<details>
<summary>本轮早期记录存档（修改前基线和实施过程，非当前待办）</summary>

最后更新：2026-09-09（用户已授权连续实施；M01～M06 已落地主要改动，M07/M08 实施与验收中）

## 当前进展 · 2026-09-09

**下一位 Agent 先读本节、[悬浮交互升级与整改计划](docs/FLOATING_UI_REMEDIATION_PLAN.md) 及 [本轮全面审计](docs/AUDIT_2026-09-09.md)。这是当前项目事实与下一步入口。** 后文按日期保留历史过程；历史“已完成/未解决/验证通过”不能覆盖本节，也不能直接当作今日待办。

- **Git。** 开工时工作区干净，位于 `main`；已 `fetch --prune origin`、`pull --ff-only origin main`，远端没有新提交。`HEAD` 与 `origin/main` 均为 `cc352cfb16b30fb16b84eec9dff74aa1ea23b8b8`。
- **版本。** 扩展源码为 **0.6.0**，服务端源码为 **0.1.15**，产物尚在最终验证中。未在本轮核对生产运行的准确版本。
- **功能状态。** T-01～T-20、T-14c 与后续 0.5.73 改动已有实现；不要重新执行整套旧重构。此次发现的是当前同步、恢复和验证链路的遗漏，不代表旧实现全未完成。
- **商业化判断。** 视觉体系已形成；同步一致性、异常恢复和状态可信度仍有发布前必须处理的问题，不能宣称稳定性、完整备份或安装态验收全部完成。

### 最新产品方向与计划状态

- **用户新增要求：** 由侧栏主入口升级为悬浮球 + 菜单，参考另一个 Git 项目 NexAlign，并尽量使用成熟组件库统一整套 UI；本轮要求制定修改计划。
- **选型与形态：** 计划采用 **HeroUI 3**，以 **3.2.4** 为接入基线；产品分为悬浮球、收藏/AI/设置浮动菜单、完整收藏库。菜单按需加载，复杂整理、报告、图谱和恢复保留大页面空间。所有界面进入共享组件迁移范围。
- **参考已核实：** `/Users/nefish/Desktop/Coding/LayerScope` 实际产品为 NexAlign。已 fetch `origin`；`codex/nexalign-floating-ui` 与上游一致，提交 `2a7bd4ac0643010d82021b8877f720bcdcecfb82`。读取桌面 52px 球、独立扩展 iframe、位置及身份机制，查看仓库保存的参考截图；没有修改、安装或重跑该项目。
- **架构方向：** 轻量网页宿主 + 隔离的扩展 iframe，共用正式业务层。重点处理页面身份绑定、草稿持久化、断连恢复、截图前隐藏自身 UI。无法注入的页面通过扩展图标进入完整收藏库。
- **当前状态：** 用户已明确授权连续完成实施。[八批计划](docs/FLOATING_UI_REMEDIATION_PLAN.md) 已补充 G01～G18 可复核目标，以及逐张实际截图的视觉审核要求。**M01～M06 主要代码已落地；当前实施 M07 恢复、图谱入口并完成 M08 回归和实际截图审核。** 下方原审计结果仍为修改前基线。
- **实施分支：** `codex/aarre-floating-ui-20260909`，从审计基线创建，保留全部未提交审计材料。当前由主 Agent 串行修改验证、数据层和 UI；其他任务暂勿同时修改本工作区。

### 当前实施记录（持续更新）

- M01：时钟 fixture、预览 Chrome 事件接口、UI 审计失败判定、依赖漏洞修复完成。
- M02/M03：数据与同步游标事务、字段时钟和清空意图、迟到响应、封面版本校验、图片候选上传和原子切换、凭证刷新幂等恢复、部分失败状态、主动重试、AI 断连和部分回答恢复已落地。服务端迁移 `009_recoverable_transfers.sql` **尚未部署**。
- M04～M06：HeroUI 3.2.4 真实控件接入全套 UI，浮动宿主 + 独立 iframe、来源校验、位置调整、截图隐藏 lease、收藏/AI/设置与右键/快捷键入口已落地；账号绑定的完整备份同意、暂停备份、每网站隐藏与重置菜单已落地。编辑及 AI 输入草稿本机保存。
- M07：完整 archive v2 导出、文件/逐图哈希校验、可审阅导入、独立目录恢复、ID 映射、重复导入幂等与中断续接已落地；收藏库增加设置和备份入口，主题图谱增加键盘可达的主题收藏列表。正在视觉收口与边界验证。
- **最近一次整套测试：根 93 文件 / 499 项通过，服务端隔离 PostgreSQL 27/27 通过。** 后续仍有视觉与边界改动，终验会再次更新。专项悬浮身份/几何/截图 25 项、archive 6 项、worker 压缩等价性 3 项通过。
- **最近构建：通过，后台 298,626 bytes，低于 330,000 bytes 硬门。** 构建对重复字符串与公开字段名做常量复用，字段名不变、不使用 eval；Chrome 序列化注入函数整段排除，避免捕获外层变量。保持单文件 MV3 worker。
- **实际视觉：** 正在 Ego 同一任务空间 5 检查正式 UI 的 DEV 数据场景，已发现并修复悬浮搜索布局缺少密度属性、管理页 HeroUI Tab 默认宽度导致每项占满导航。中间截图位于 `docs/verification/2026-09-09/intermediate/`。另有明确标识的 `host-harness.html` 用生产宿主/iframe UI + 浏览器 API 测试替身检查浮动外观与交互，**不是 installed Chrome 验收**。
- **剩余：** 最终逐图检查和图库、host 生命周期与长内容回归、archive 异常恢复边界、公开文档与隐私一致性、清理旧依赖、完整最终检查/产物打包。Mac 锁屏；Ego 不支持扩展管理协议，因此实际已安装 Chrome、外部 Provider、生产服务端迁移/COS/双设备仍需分别标明未验收。

### 原审计阶段已完成（修改前基线）

- 拉取并核对最新 Git，阅读当前进展、架构、历史审计和 PRD。
- 审查客户端/服务端、收藏与增强队列、同步与恢复、隐私说明、依赖及验证体系。
- 重跑标准检查、两端构建、真实本地 PostgreSQL 集成测试、产物与素材检查。
- 使用现有 DEV fixture 完成管理页日夜主题、420px 窄窗口、编辑、搜索空状态、筛选排序、五个页面及键盘焦点抽样；保存并逐张检查 **12 张截图**。
- 独立复现 **10 个未满足应有行为的故障场景**；输出 **19 项审计发现**、影响边界、修复顺序与验收条件。
- 交付 [完整报告](docs/AUDIT_2026-09-09.md) 和 [复现/截图/日志证据](docs/audits/2026-09-09/README.md)。只维护审计材料和本文件，没有修改产品源码、版本或依赖，没有提交、推送或发布。
- 根据用户后续要求，核对 NexAlign 最新悬浮分支及 HeroUI/Mantine/shadcn 官方资料，交付 [悬浮球与整套 UI 整改计划](docs/FLOATING_UI_REMEDIATION_PLAN.md)。计划研究未另行重跑产品测试，沿用上方审计基线；仅新增计划和更新本文件。

### 原审计问题索引（修复状态以上方实施记录为准）

- **同步正确性：** A01 游标先于数据提交；A02 更新后删除被重新写回；A05 迟到响应覆盖新备注；A06 旧设备上传旧封面；A10 清空备注/标签不进入协议。
- **异常恢复与真实状态：** A03 部分失败显示已同步；A04 手动重试被退避直接跳过；A07 临时断网刷新凭证时退出登录；A08 Agent 连接断开一直等待；A09 最近更改只露出 12 条；A18 缺少本地 archive 导入恢复闭环。
- **产品与验证：** A11 登录即完整同步与公开说明不一致；A12 侧栏开发预览因 `tabs.onActivated` 缺失无法启动；A13 测试固定日期已过期；A14 UI 脚本跳过失败场景仍正常退出；A15 当前存在已公告依赖风险。
- **视觉：** A16 日夜主题辅助文字对比度不足；A17 图谱缺少键盘和主题收藏入口。窄窗导航、编辑操作区、报告与重新发现还有精修建议。
- **文档：** A19 根进展的当前入口和旧状态标识已校正；README、引导与公开政策内容修订尚未完成。
- **外部验收：** 审计时电脑控制返回锁屏，真实 Chrome 安装态未验收；开发侧栏本身启动失败。完整双设备同步、真实 Provider、COS 图片恢复、全新配置文件恢复、300/1,000/2,000 条真实数据规模与正式商店/授权状态仍需单独验证。

### 原审计验证情况（修改前基线）

- 本轮没有实施产品缺陷修复。已解决的是进展文档当前入口混乱，并建立可复核证据；历史功能修复继续保留在各日期记录中。
- 根 `npm run check`：Node、设计 token、TypeScript 通过；**450/451 项、86/87 文件通过**。唯一原有失败：撤销 fixture 在 2026-08-29 已到期；属于测试时钟问题，不是确认有效期内撤销失效。
- 单独 `npm run build`：通过，全部输出 JavaScript 语法检查通过。`dist/background.js` **329,976 bytes**；不等于已安装 Chrome 正常。
- 服务端类型检查、构建通过；新建独立本地 PostgreSQL 测试库的集成测试 **25/25 通过**，对象存储仍是测试替身。
- 本轮专项 `.cases.ts`：**10 项断言失败，确认相应缺陷**。它们独立于标准测试，不参与平常 `*.test.ts` 收集；命令和解释见证据 README。
- `verify:store-assets`：5 张 JPEG 与视频格式通过；`verify:artifacts`：无 `outputs/`，**跳过**，不能写成 ZIP 已验收。
- npm audit：根依赖 5 个条目（3 high / 2 moderate），服务端 3 个（1 high / 2 moderate）；部分为开发/未使用依赖，未证实生产可利用。
- 公网 `/ready` 当时返回 `{"ok":true}`；公开隐私政策已读取。未据此宣称账号同步、OAuth、备份恢复或准确生产版本已验证。

### 下一步计划

按 [统一实施计划](docs/FLOATING_UI_REMEDIATION_PLAN.md) 推进，不另建进展文档：

1. **M01：** 修复预览、测试时钟、UI 审计失败判定和依赖风险，建立可靠回归基线。
2. **M02：** 修复同步提交、删除顺序、迟到响应、图片版本竞争和字段清空。
3. **M03：** 修复同步状态、手动重试、登录恢复、AI 断连和最近更改可达性。
4. **M04：** 建立 HeroUI 组件与日夜主题、焦点、排版及弹层规范；此批只依赖 M01，可在同步整改期间安排。
5. **M05：** 实现真实悬浮宿主与收藏主路径，完成来源绑定、位置恢复、截图隐藏和生命周期验证。
6. **M06：** 迁移全部侧栏功能及原生星标/右键/快捷键入口，统一完整同步说明和首次启用流程。
7. **M07：** 统一收藏库五页 UI，补图谱入口、窄窗操作与完整 archive 恢复闭环。
8. **M08：** 完成真实 Chrome、双客户端、Provider、对象存储、升级恢复与产物验收，再判断发布条件。

### 当前占用与接力约束

审计与计划已交付，实施正在本工作区进行。当前占用：根进展、统一实施计划、验证脚本、测试和依赖文件；随后按批次切换到同步与 UI 模块。审计证据保持原样，后续结果单独保存。用户现在使用手机，待回到电脑亲自体验；不因此跳过可执行的自动化和逐图审核。

继续保留既定产品决定：原生字段交给 Chrome；智能层由 Aarre 管理；完整备份为唯一云端范围；不恢复旧 Supabase 方案、透明 surface、毛玻璃吸顶条、设置页封面风格切换或常驻白色提示条。IndexedDB schema 变更和同一文件修改仍应串行协调。
...
以下是各版本当时的实施与验证记录，保留用于追溯。旧 [2026-08 审计](docs/AUDIT_2026-08.md) 与 [重构 PRD](docs/PRD_REBUILD_2026-08.md) 只用于理解背景及已接受的约束；当日测试数、生产版本、外部阻塞和未修清单均以其记录日期为限。
...
- **用户目标。** 点击收藏后尽快完成网站标识与截图；目标是不需要原地停留太久。手动扫描只保留首次使用回填。
- **耗时基线（代码实测）。** 截图链路：250ms 预热 → 页面稳定等待（字体就绪 + 900ms 静默，页面已加载时约 1s）→ 截取保存约 0.3-0.5s，典型 **1.5-2.5s**；图标链路：HTML 读取 0.2-0.8s → 常规路径串行探测（9 次 HEAD，0.5-2s，最大浪费点）→ 并行下载解码 0.2-0.8s，典型 **1.5-4s**。两条链路并行，总体 2-4s；期间标签页必须保持前台，切走则截图转为下次前台访问补拍。
- **提速。** ① 常规图标路径 9 个 HEAD 全部并行、命中候选仍按原始优先级返回，典型省 0.5-1.5s；② 扩展保存路径用页面已声明的 favicon 作为首选候选（新 `capture-favicon` 来源），命中即跳过整轮 HTML 读取，典型省 0.3-1s，失败仍回退完整扫描；③ 截图稳定等待保持不变（900ms 静默是避免截到中途渲染的质量底线）。
- **验证。** 新增“种子候选跳过 HTML 读取”测试，`site-icon-immediate` 共 5 项全绿；完整 83 文件 / 441 项测试通过（`snapshot-backfill-control` 焦点断言偶发失败与本次无关，单独重跑通过）；`npm run check` 其余阶段全绿；`dist/manifest.json` 为 0.5.69，后台产物含 `capture-favicon` 与 `ensureSiteBrandForResource`。
- **真人验收门。** 重载 0.5.69 后实测：点星标收藏一个已加载完成的页面，侧边栏图标应在 1-3s 内出现；截图完成约 2s（状态提示“截图已更新”）。若页面仍在加载，等待会顺延到加载完成 + 约 1s。

## 2026-08-05 · 网站标识接入收藏保存路径（0.5.68）

- **用户目标。** 无论用扩展自己的入口还是 Chrome 星标收藏，点击后都应尽量自动完成 AI 增强、截图与网站标识抓取；手动全目录扫描只保留给首次使用的历史收藏回填。
- **现状核查。** AI 与截图两条路径早已自动排队（有条件：AI Key、隐私规则、页面前台稳定）；但网站标识完全不在保存链路里，只能等全目录扫描或版本失效重抓，这正是此前“收藏后图标迟迟不出现”的原因。
- **修复。** 图标管线新增 `ensureSiteBrandForResource`：缓存新鲜直接跳过；缺失/过期时读取公开 HTML 提取图标候选（apple-touch-icon、SVG、manifest、常规路径、主域别名、公共服务兜底），解码后落库并广播 `SITE_BRANDS_UPDATED`；同主机并发去重、同域 750ms 限速、正文读不到（登录墙/网络失败）不阻塞仍走兜底，失败只返回 false 不影响收藏。
- **接线。** 扩展保存路径（`bookmark-save.ts`）与 Chrome 星标路径（`resources.ts` 的 onCreated 增强分支）在资源落库后立即后台触发；隐私保护页面不发起读取，与扫描管线行为一致。`siteIconHandlers` 创建顺序提前以注入两个保存处理器。
- **验证。** 新增 4 项测试（新鲜缓存零网络、抓取落库并广播、同主机并发去重、保存路径接线守卫）；`npm run check` 全绿：设计 token、TypeScript、83 个测试文件 / 440 项测试、生产构建、JS 产物语法检查；`dist/manifest.json` 为 0.5.68，后台产物含新逻辑。
- **真人验收门。** 在 `chrome://extensions` 重载 0.5.68 的 `dist/`，用两种方式各收藏一个新站点，观察侧边栏图标是否在几秒内出现（无需手动扫描）；受保护站点（内网/银行/支付/医疗）应保持分类封面。
...
- **列表项复用书签栏样式。** 最近动作与历史会话条目直接使用 `.bookmark-row`，共享书签栏的透明常态、整行 hover/active、圆角和间距；移除这两类条目原有的常驻卡片底色、边框和独立 hover 绘制。
- **AI 标签输入。** 点击 PlusIcon 后出现的输入框继续使用标签 chip 的浅灰底和圆角，去掉描边、焦点边框与阴影，回车/逗号仍会生成标签。
- **删除入口与确认。** 侧边栏及管理页编辑器底部删除入口移除垃圾桶图标；确认删除按钮统一使用 `--danger-ink` 白色文字，保留确认说明区域的提示图标。
- **验证状态。** `npm run check` 全绿：设计 token、TypeScript、82 个测试文件 / 433 项测试、生产构建和 JavaScript 产物语法检查通过；`package.json`、`public/manifest.json`、`dist/manifest.json` 均为 0.5.65。运行态“设置更多”“收藏库编辑弹窗”“收藏库删除确认”三场景 `ui:audit` 合计 0 项。

## 2026-08-05 · 截图反馈修复与样式审计（0.5.64）

- **侧边栏列表。** 去掉滚动容器顶部 padding，运行态测得吸顶文件夹 `rowTop === contentTop`、gap 为 0；“我的书签”标题固定为纯文本，移除头像、hover 和点击账号设置功能，设置仍由右侧齿轮进入。
- **AI 设置。** 服务商切换和模型输入合并为“模型与服务”下拉；已保存 Key 的只读态会同时禁用该下拉，点击“编辑”后两者一起解锁。设置页登录、扫描、继续、引导与隐私操作统一到 tertiary 按钮语义。
- **列表交互。** 历史会话去掉改名功能，整行共享 hover/active surface；最近动作去掉“最近的更改”标题和常驻卡片背景，默认透明、整行 hover。新增 `--row-*`、quiet danger、readonly field 和 icon 尺寸 token。
- **编辑收藏。** 标签添加改为标签流末尾的 PlusIcon 小按钮，点击后原地变成同尺寸输入 chip，回车生成标签；侧边栏和网页端删除入口均改为与账号“断开”一致的中性 quiet danger 样式，图标收敛到 `--icon-sm`。
- **审计报告。** 新增 [`docs/STYLE_AUDIT_2026-08-05.md`](docs/STYLE_AUDIT_2026-08-05.md)。主要遗留是双 icon 系统、`--ink-faint` 对比度不足、几何 token 门未覆盖、页面级控件 CSS 偏多、触控命中区偏小、onboarding 与设置 AI 选择模式不一致，以及明确的无引用 CSS/token。
- **验证。** `npm run check` 全绿：设计 token、TypeScript、82 个测试文件 / 433 项测试、生产构建和 JS 语法检查通过；`dist/` 已重建为 0.5.64。亮色与暗色完整 `ui:audit` 均为 0 项；侧边栏吸顶实测 gap=0；管理页编辑/删除二次审计 0 项。
- **真人验收门。** 仍需在 `chrome://extensions` 重载最新 `dist/`，用真实账号验证 Key 冻结/编辑、真实历史会话整行 hover、多个最近动作、长标签添加与吸顶滚动。开发预览与自动审计不能冒充安装态真人验收。

## 2026-08-05 · Round 3 根因与体验收口（0.5.63）

- **R1 下拉透明已根治。** 侧边栏与管理页 Tailwind 入口显式扫描 `surface-classes.ts`，生产 CSS 已生成 `bg-surface-*` / `shadow-surface-*`；新增产物防回归测试。
- **R3 会话覆盖已封堵。** 只有包含完整 Assistant 答案的稳定会话才进入同步；云端恢复会比较 `updatedAt`，旧版本和空占位不会覆盖本地新答案。已损坏的历史答案无法恢复。
- **R2 采用用户选择的 A 方案。** 保留滚动末端渐隐，展开文件夹吸顶条改为不透明实色，并移除无效毛玻璃代码，避免文字透叠。
- **Agent 引用改为正文内联。** Prompt 要求具体收藏使用 Markdown 原始网址；最终 `sources` 只保留正文真正引用的收藏；正文链接渲染站点图标 chip，底部仅显示未在正文引用的“其他相关收藏”。
- **T-1～T-8 已完成。** 账号操作右置且“断开”为中性按钮红色 hover；显示页只保留公共 favicon 开关；“更多”改为“最近动作”，引导和隐私成为独立行，数据导出迁到隐私页；已配置 API Key 显示只读掩码并通过“编辑”进入替换态；编辑收藏删除解释性文案并收紧间距；添加收藏下拉可真实创建 Chrome 文件夹并自动选中；搜索框移除回车角标；普通 Toast 为暗色胶囊、2 秒无关闭按钮，错误 6 秒且保留关闭按钮。
- **验证。** 完整 `npm run check` 全绿：设计 token 检查、TypeScript、81 个测试文件 / 427 个用例、生产构建及 JavaScript 产物语法检查全部通过；`dist/` 已按 0.5.63 重建。
- **真人验收门。** 需在 `chrome://extensions` 重载最新 `dist/`，实测所有下拉层、长列表吸顶、添加收藏中新建文件夹、三家 Provider 的 Key 编辑态、Agent 内联引用、Toast 时长与跨设备会话不倒退。自动化不冒充这些安装态检查。

## 2026-08-05 · Round 3 审核（历史审核输入，已实施）
...
**历史工作区快照：0.5.61（不代表 2026-09-09 当前状态）。** 同步链路近期收口：0.5.61 服务端 0.1.11 的 PUT 增加 `x-cos-metadata-directive: Replace`（覆盖旧对象强制替换元数据，修复 422 校验失败根因），云端 236 条资产已按用户要求“以本机为准”全部标记删除并排队物理清理、用量归零，客户端对账后由本机全量重传；0.5.60 客户端上传遇 409 自动换新 ID 重试，服务器软删除 181 条 assetId 与内容不匹配的记录；0.5.59 图片封面自动应用到同站现有收藏（不再产生根目录重复书签）；0.5.58 敏感网页保护开关可点击并有明确反馈；0.5.57 编辑保护开关如实显示自动隐私保护（`autoProtected` 锁定）；0.5.56 删除卡片局部移除 + 站点图标强制云端恢复；0.5.55 编辑/删除刷新跳过全量同步 + 封面会话级缓存；0.5.54 删除保持滚动位置、favicon 质量门槛放宽（16px/3:1/0.01，图标版本 7）、Google 账号区块置顶；0.5.53 清理同图多网址快照；0.5.52 AI 格式诊断 + JSON 容错；0.5.51 AI 对话修复；0.5.50 同一网址多条收藏根因修复（确定性 bookmarkItemId + 去重）。服务器当前 0.1.11（release `20260803-metadata-v29`）。当前 `dist/` 为 cloud-enabled 0.5.60 正式构建（带 key）。F14 生产 API、数据库、COS/CAM、Google Web OAuth、DNS/TLS、定时备份、两分钟健康巡检、独立 GlitchTip project 与含 SSH Key 的加密恢复包已经部署；Google 品牌审核、卸载重装恢复和正式 Web Store ID 尚未完成。

## 历史阶段摘要（0.5.34 及更早状态，非当前进展）

以下保留当时叙述；版本、范围和验证状态均已由顶部当前摘要取代。

**当时的状态校正：** 上方历史摘要中的 0.5.33 文字已由本轮 0.5.34 构建取代；当前 `dist/manifest.json` 为 0.5.34，且显式连接 `https://sync.nexvoice.cc`。本轮最终验证为 329 项测试通过。
...
## 历史功能清单（早期架构，不能视为当前能力清单）

其中 Supabase、云端 AI 与旧权限描述已经退出当前架构，仅保留历史记录。
...
## 历史计划（2026-08-03，相关重构已有实现）

以下为当时计划与决策输入，不是今日待办；不得据此重新询问已解决的迁移决策。当前整改见文档顶部及 2026-09-09 审计。

### 当时主线：按 `docs/PRD_REBUILD_2026-08.md` 执行 T-01 → T-20
...
**当时的 P0 判断（相关修复已在后续版本实施）**：当时用户每天都在受这 4 个 bug 影响，且改动量很小（合计不到 200 行）。
...
这部分保留当时问题与旧外部状态，部分已解决或作废；今日缺陷、生产证据边界和验收待办只看文档顶部。

**2026-08-03 审计当时的根因缺陷（对应 T 项已有后续实现；今日遗漏见 2026-09-09 报告）**
...
**历史云端状态（F14；下述 DNS/公开入口未开通等判断已经过时）**
...
## 历史验证情况（不可作为当前全绿或已安装证据）

当前结果为 450/451、服务端 25/25，详情见顶部。以下保留各版本当日输出，含已过时的依赖风险与 UI 结论。
...
当前占用见文档顶部；下列 F14 独占、Supabase 路径和旧文件行数不再是本轮编辑限制。
