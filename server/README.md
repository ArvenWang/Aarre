...
本轮加入 `009_recoverable_transfers.sql`：同一次凭证刷新可在 5 分钟内恢复响应，图片先上传到独立候选对象，通过大小、摘要、服务端加密和版本校验后才切换正式对象。先运行迁移，再启动新服务。旧客户端仍支持原有上传调用；0.6.0 客户端启用额外版本校验。

本地 PostgreSQL 集成验证使用独立测试库；对象存储是测试替身。生产数据库迁移、公开隐私页部署、真实 COS 和双设备图片恢复必须单独验收，不能由本地测试代替。
